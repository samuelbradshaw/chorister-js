...
ChScore.prototype._optimizeMusicXml = function (musicXml) {
  ...
  if (hasRests) {
    for (const measure of parsed.querySelectorAll('measure')) {
      const notesByVoice = this._groupBy(measure.querySelectorAll('note'),
        note => note.querySelector('voice')?.textContent?.trim() ?? '');
      for (const notes of notesByVoice.values()) {
        if (notes.length !== 1) continue;
        const rest = notes[0].querySelector('rest:not([measure])');
        if (!rest || notes[0].querySelector('type')?.textContent?.trim() !== 'whole') continue;
        rest.setAttribute('measure', 'yes');
        notes[0].querySelector('type').remove();
        changed = true;
      }
    }
  }

  // A grace *rest* is an engraver's invisible spacer, but Verovio times it from its <type>, which
  // can run a 3/4 measure to 4.5 quarters -- and an over-full measure shifts every measure number
  // after it, the numbers a stored template transfers by (see _measuresFrom). Dropped: they are
  // invisible and sound nothing.
  if (hasGrace) {
    for (const note of parsed.querySelectorAll('note')) {
      if (!note.querySelector('grace') || !note.querySelector('rest')) continue;
      note.remove();
      changed = true;
    }
  }
  ...
  if (hasLyrics) {
    changed = this._moveHeldMelodyLyrics(parsed) || changed;
    changed = this._normalizeTypedHyphens(parsed) || changed;
    changed = this._mergeMarkOnlyLyrics(parsed) || changed;
    ...
    this._readSpacedElisions(parsed);
  }
...
// Which of this score's elisions print a space rather than a breve, as the pair of syllables
// each one falls between. MusicXML says which in the elision's own text -- a breve joins two
// vowels sung on one note, a space is typography (French prints "ne !") -- but Verovio's
// importer writes MEI's breve connector for both and keeps none of the text. So it is read
// here, while the MusicXML is in hand, and applied to the MEI in _spaceElisionConnectors.
ChScore.prototype._readSpacedElisions = function (parsed) {
  const spaceOnly = /^\s+$/;
  const near = (element, direction) => {
  ...
  for (const elision of parsed.querySelectorAll('lyric > elision')) {
    if (!spaceOnly.test(elision.textContent ?? '')) continue;
    const before = near(elision, 'previousElementSibling');
    const after = near(elision, 'nextElementSibling');
    if (!before || !after) continue;
    this._spacedElisions.add(this._elisionKey(before.textContent, after.textContent));
  }
}

// The two syllables an elision falls between, which is all the MusicXML and the MEI share:
// Verovio keeps neither the elision's text nor an id of its own for it.
ChScore.prototype._elisionKey = function (before, after) {
  return `${(before ?? '').trim()}\u0000${(after ?? '').trim()}`;
}

// An elision that printed a space, drawn as one: @con goes back to MEI's space connector,
// which is what it means everywhere else in the document. An elision that really is one keeps
// its breve -- two vowels sung on one note are what the mark is for.
ChScore.prototype._spaceElisionConnectors = function (meiParsed) {
  if (!this._spacedElisions?.size) return;
  for (const syl of meiParsed.querySelectorAll('syl[con="b"]')) {
    const next = syl.nextElementSibling;
    if (next?.nodeName !== 'syl') continue;
    if (!this._spacedElisions.has(this._elisionKey(syl.textContent, next.textContent))) continue;
    syl.setAttribute('con', 's');
  }
}

// A mark given a lyric element of its own on a note that already has one: French sets a space
// before "!" and ";", and Russian opens a line with a dash, both hung on the note as a second
// <lyric> of the same number. MusicXML doesn't say how to read two of one number on one note,
// and only one of them survives, so the syllable beside the mark was being lost.
//
// Joined only where one of the two carries no letters at all -- that is typography, the way a
// typed hyphen or a spacing elision is. Two real syllables are left alone, since which of them
// to keep is a different question.
ChScore.prototype._mergeMarkOnlyLyrics = function (parsed) {
  const letters = this._patterns.letterOrNumber;
  const textOf = (lyric) => Array.from(lyric.querySelectorAll('text'))
    .map(text => text.textContent ?? '').join('');

  let changed = false;
  for (const note of parsed.querySelectorAll('note')) {
    const lyricsWithText = Array.from(note.children)
      .filter(child => child.localName === 'lyric' && textOf(child).trim());

    for (const lyrics of this._groupBy(lyricsWithText,
      lyric => lyric.getAttribute('number') ?? '1').values()) {
      if (lyrics.length < 2) continue;
      const syllables = lyrics.filter(lyric => letters.test(textOf(lyric)));
      // Two syllables on one number is the different question; none is a note of marks alone
      if (syllables.length !== 1) continue;

      const syllable = syllables[0];
      const texts = Array.from(syllable.querySelectorAll('text'));
      if (texts.length === 0) continue;
      for (const mark of lyrics) {
        if (mark === syllable) continue;
        const before = lyrics.indexOf(mark) < lyrics.indexOf(syllable);
        const target = before ? texts[0] : texts.at(-1);
        const markText = textOf(mark).trim();
        target.textContent = before
          ? `${markText} ${target.textContent ?? ''}`
          : `${target.textContent ?? ''} ${markText}`;
        // An extender belongs to the note, not to the element it was written on
        for (const extend of Array.from(mark.querySelectorAll('extend'))) syllable.appendChild(extend);
        mark.remove();
        changed = true;
      }
    }
...
// A hyphen typed into the lyrics rather than left to the engraver: a syllable at the end of a
// repeat ending printed "a-", so the reader can see the word carries on into the other ending,
// written as <syllabic>end</syllabic><text>a</text> plus elided empty syllables holding a lone
// "-". MusicXML says "the word continues" with <syllabic>, which is what becomes @wordpos, so
// read literally the word ends here -- "Gethsemane." comes out "Gethsema ne." The hyphen is
// dropped and the syllable it was hung on says the word continues.
ChScore.prototype._normalizeTypedHyphens = function (parsed) {
  const hyphens = new RegExp(`^[${this._hyphenCharacters}\\s]+$`);
  const trailing = new RegExp(`[${this._hyphenCharacters}]+$`);
  let changed = false;

  for (const lyric of parsed.querySelectorAll('lyric')) {
    const texts = Array.from(lyric.querySelectorAll('text'));
    if (texts.length === 0) continue;

    // The last one that says anything. Everything after it is the hyphen and the empty
    // elided syllables an engraver hangs it on, which carry no word of their own.
    const saysNothing = (text) => {
      const content = (text.textContent ?? '').trim();
      return content === '' || hyphens.test(content);
    };
    let last = texts.length - 1;
    while (last >= 0 && saysNothing(texts[last])) last -= 1;
    const typed = last < texts.length - 1
      || trailing.test((texts[texts.length - 1].textContent ?? '').trimEnd());
    if (last < 0 || !typed) continue;

    // Everything after the last real syllable, its <elision> and <syllabic> included
    let node = texts[last].nextSibling;
    while (node) {
    ...
    const withoutHyphen = (text.textContent ?? '').trimEnd().replace(trailing, '');
    if (withoutHyphen !== text.textContent) text.textContent = withoutHyphen;

    // And the syllable says what the hyphen was drawn to say
    const syllabic = text.previousElementSibling?.nodeName === 'syllabic'
      ? text.previousElementSibling : null;
    const continues = syllabic?.textContent === 'begin' || syllabic?.textContent === 'middle';
    if (!continues) {
      const opens = !syllabic || syllabic.textContent === 'single';
      if (syllabic) {
        syllabic.textContent = opens ? 'begin' : 'middle';
      } else {
        const element = parsed.createElement('syllabic');
...
ChScore.prototype._getScoreMetadata = function (meiParsed, scoreId, lang) {
  ...
  // Set as prose, wrapped to the width of the page rather than to the tune, so two sung lines
  // arrive as one printed line. Its words are the verse's; its line endings are the column's.
  ...
  const looksLikeStanza = block => {
    ...
    // guard is there to keep performance notes out, and a wrapped verse trips it.
  ...
  for (const block of textBlocks) {
    ...
    // Reported, so _printedStanzaLines can leave a wrapped stanza's line endings out of where
    // the music divides
    if (block.type === 'stanza') block.wrapped = wrapped.has(block);
  }
...
ChScore.prototype._parseAndAnnotateMei = function (scoreId, lang) {
  ...
  this._spaceElisionConnectors(this._scoreData.meiParsed);
  ...
  this._finalizeMeasures(elementsById, chordPositionIndex.qstamps);
  ...
  for (const lyricElement of this._scoreData.meiParsed.querySelectorAll('verse')) {
    ...
    // A lyric line is named by the row it's drawn on, so an id and @n always agree; which
    // verse a row is, with help text counted out, is _verseLineNumber's to say. Help text
    ...
    if (!lyricElement.hasAttribute('ch-help-text')) {
      const lyricLineId = `${staffNumber}.${lyricElement.getAttribute('n')}`;
  ...
  // After the expansion walk, so each marker can say where it is played as well as where
  // it is printed
...
ChScore.prototype._resolveKeySignatures = function () {
  ...
  // The key the score opens in: its first keySig, or the scoreDef's own attributes
  const keyElement = keySignatureElement ?? scoreDefElement;
  const defaultKeySignatureId = keyElement ? this._keySignatureIdOf(keyElement, tonality) : null;
  const defaultKeySignatureInfo = keySignatures?.[defaultKeySignatureId] ?? null;
  if (!defaultKeySignatureId) {
    const sig = keyElement?.getAttribute('sig') ?? keyElement?.getAttribute('key.sig');
    console.error(`Error: Unrecognized key signature (sig: ${sig}, mode: ${tonality}).`);
  }
  this._scoreData.keySignatures = this._getKeySignatureIds(tonality);

  // Get nearby key signatures
  const nearbyKeySignatureIds = defaultKeySignatureId ? Object.keys(keySignatures) : [];
  ...
  this._scoreData.keySignatureInfo = defaultKeySignatureId ? {
...
ChScore.prototype._applyMeiDefinition = function (element, inForce) {
  ...
  inForce.timeSignature = [
    Number.parseInt(element.getAttribute('count') ?? element.getAttribute('meter.count')
      ?? inForce.timeSignature[0]),
...
// the rebuilt document can be read the same way the score is. It stands for the same music, so
// it keeps the source's place; `span` is where a range clipped it (its own startQ, endQ,
// durationQ and firstChordPosition). Its barline is settled with the rest in _updateMei.
ChScore.prototype._addRebuiltSubMeasure = function (element, sourceId, span = null) {
  const source = this._scoreData.subMeasuresById[sourceId];
  const subMeasureId = element.getAttribute('xml:id');
  if (!source || !subMeasureId) return;
  this._scoreData.subMeasuresById[subMeasureId] = { ...source, ...span, subMeasureId };
}

// The measures a run of sub-measures makes up, in order and numbered. A measure is sometimes
// written in more than one <measure> -- a section can close part-way through a measure and the
// next pick the rest up ("All Things Bright and Beautiful", where the chorus ends three beats
// in) -- so this is the one place that decides which of them belong together.
ChScore.prototype._measuresFrom = function (subMeasures) {
  const measures = [];
  subMeasures.forEach((subMeasure, index) => {
    // Grouping is settled before anything is split, so the sub-measure after this one is a
    // whole measure of the score, never a piece of one
    const previous = measures.at(-1);
    const [count, unit] = subMeasure.timeSignature;
    const completeDurationQ = count * (4 / unit);
    const nextDurationQ = subMeasures[index + 1]?.durationQ;
    const continues = previous != null
      // Only a measure still short of its meter, and in that meter: one already full is
      // complete, and the end of a 6/8 section and a 4/4 pickup after it are two measures
      && previous.timeSignature[0] === count && previous.timeSignature[1] === unit
      && previous.durationQ < completeDurationQ
      // An over-full sub-measure continues nothing; _optimizeMusicXml repairs those it can
      && subMeasure.durationQ < completeDurationQ
      // The score's last sub-measure is the pickdown, which closes a measure of its own
      && nextDurationQ != null
      && subMeasure.rightBarLine !== 'invis'
      // A short sub-measure that pairs with the measure after it is that one's near half
      && nextDurationQ + subMeasure.durationQ !== completeDurationQ;
    if (continues) {
      ...
      measures.push({
        ...
        rightBarLine: subMeasure.rightBarLine, // As the score writes it, where its last sub-measure ends
  ...
  // Each measure is full, or short of its meter: a pickup leading into a section or a pickdown
  // closing one. The first only leads in and the last only closes; one between closes a section
  // where a barline that means something, or a change of meter, follows it. Only a pickup the
  // score opens on is 0 -- the engraved @n is no use: scores here write "0", "X1", "X2".
  let counter = 0;
  measures.forEach((measure, index) => {
    const [count, unit] = measure.timeSignature;
    const next = measures[index + 1];
    const endsSection = !next
      || ['dbl', 'dbldashed', 'dbldotted', 'dblheavy', 'heavy', 'end', 'rptstart', 'rptend',
        'rptboth'].includes(measure.rightBarLine)
      || next.timeSignature[0] !== count || next.timeSignature[1] !== unit;
    if (measure.durationQ >= count * (4 / unit)) measure.measureType = 'full';
    else if (index > 0 && endsSection) measure.measureType = 'partial-pickdown';
    else measure.measureType = 'partial-pickup';
    measure.measureNumber = index === 0 && measure.measureType === 'partial-pickup'
      ? '0' : String(++counter);
  });
  return measures;
}

// Builds _scoreData.measures -- settled only after the walk above, since whether a sub-measure
// continues a measure depends on the one after it -- and writes each sub-measure's place, and its
// measure's number as @n, back onto it. `qstamps` places the pieces a split leaves behind,
// which have no timemap of their own.
ChScore.prototype._finalizeMeasures = function (elementsById, qstamps) {
  ...
  const measureElements = Array.from(this._scoreData.meiParsed.querySelectorAll('measure'));
  const subMeasures = measureElements
    .map(element => this._scoreData.subMeasuresById[element.getAttribute('xml:id')]);
  const measures = this._measuresFrom(subMeasures);
  // Split what can be split, now that the grouping is settled: the pieces join the measure
  // they were cut from, so nothing has to work out again which <measure> elements go together.
  // A measure's own span is untouched -- the pieces only divide it.
  const incoming = this._incomingSpans(measureElements, elementsById);
  // A span's @tstamp2 counts <measure> elements, which a split adds to, and its beat from the
  // start of one a split may have divided -- so where each one ends is noted before anything
  // moves, and written back once everything has
  const tstamp2Ends = [];
  measureElements.forEach((element, index) => {
    for (const spanner of element.querySelectorAll(':scope > [tstamp2]')) {
      const [, measuresOn, beat] = spanner.getAttribute('tstamp2').match(CH_TSTAMP2) ?? [];
      const endElement = measureElements[index + Number(measuresOn)];
      if (endElement) tstamp2Ends.push({ spanner, endElement, endBeat: Number.parseFloat(beat) });
    }
  });
  const splits = new Map();
  for (const measure of measures) {
    const split = this._splitMeasure(measure, elementsById, qstamps, incoming);
    if (split) splits.set(split.element, split);
  }
  if (splits.size > 0) {
    const orderOf = new Map(Array.from(this._scoreData.meiParsed.querySelectorAll('measure'))
      .map((element, index) => [element, index]));
    for (const { spanner, endElement, endBeat } of tstamp2Ends) {
      // An end past the split point is in the piece now, counted from the piece's start
      const split = splits.get(endElement);
      const [element, beat] = split && endBeat > split.tstamp
        ? [split.piece, chRoundTstamp(endBeat - split.tstamp + 1)] : [endElement, endBeat];
      const measuresOn = orderOf.get(element) - orderOf.get(spanner.closest('measure'));
      spanner.setAttribute('tstamp2', `${measuresOn}m+${beat}`);
    }
  }
...
// Split a measure of more than four chord positions once, as near its middle as it splits
// cleanly -- one of more than six may cross a slur, tie, hairpin or word to get there -- and
// record the new piece. The measure keeps its number, span and first chord position. Returns
// the split made ({ element, piece, tstamp }), or nothing.
ChScore.prototype._splitMeasure = function (measure, elementsById, qstamps, incoming) {
  if (measure.startQ == null) return;
  const chordPositionCount = this._bisectLeft(qstamps, measure.endQ)
    - this._bisectLeft(qstamps, measure.startQ);
  if (chordPositionCount <= 4) return;

  // The point nearest the middle, among any of its sub-measures' points, as a qstamp, so it can
  // be picked however the measure happens to be written
  const middleQ = measure.startQ + (measure.durationQ / 2);
  const nearestToMiddle = (relaxed) => {
    let nearest = null;
    measure.subMeasureIds.forEach((subMeasureId, index) => {
      const subMeasure = this._scoreData.subMeasuresById[subMeasureId];
      const element = elementsById.get(subMeasureId);
      // A sub-measure that sounds nothing has no span to divide and nothing to divide at
      if (!element || subMeasure.startQ == null) return;
      // The meter the record was built with, which follows a change written anywhere before
      // it; [0, 0] where the score names none
      const meter = subMeasure.timeSignature[1] ? subMeasure.timeSignature : [4, 4];
      const tstamps = this._measureSplitTstamps(
        element, meter, incoming.get(element) ?? [], relaxed);
      for (const tstamp of tstamps) {
        const qstamp = this._tstampToQstamp(tstamp, subMeasure.startQ, subMeasure.endQ, meter[1]);
        if (!nearest || Math.abs(qstamp - middleQ) < Math.abs(nearest.qstamp - middleQ)) {
          nearest = { index, subMeasure, element, meter, tstamp, qstamp };
        }
      }
    });
    return nearest;
  };
  const inMiddleHalf = (point) => Math.abs(point.qstamp - middleQ) <= measure.durationQ / 4;
  let best = nearestToMiddle(false);
  if ((!best || !inMiddleHalf(best)) && chordPositionCount > 6) {
    const relaxed = nearestToMiddle(true);
    if (relaxed && inMiddleHalf(relaxed)) best = relaxed;
  }
  if (!best) return;

  const { index, subMeasure, element, meter, tstamp, qstamp } = best;
  ...
  const record = {
    ...
    splitFrom: subMeasure.subMeasureId,
    // The span is worked out from where the sub-measure was cut rather than asked of Verovio
    // again: a second timemap would mean loading the document into it a second time, which is
    // the slowest thing this code does
    startQ: qstamp,
    durationQ: subMeasure.endQ - qstamp,
    firstChordPosition: this._bisectLeft(qstamps, qstamp),
  };
  // The sub-measure keeps its id and its first chord position, and now ends where the piece
  // begins
  subMeasure.endQ = qstamp;
  subMeasure.durationQ = qstamp - subMeasure.startQ;
  subMeasure.rightBarLine = 'invis';
  this._scoreData.subMeasuresById[record.subMeasureId] = record;
  elementsById.set(record.subMeasureId, piece);
  measure.subMeasureIds.splice(index + 1, 0, record.subMeasureId);
  return { element: element, piece: piece, tstamp: tstamp };
}

// What reaches into each of `measureElements` from an earlier one. A spanner (@endid or
// @tstamp2: a slur, tie, hairpin...) is written in the <measure> it starts in, so one ending
// further on is only seen from there. Each gets the spans reaching it: the event one ends on
// (`endEvent`), or the tstamp it is open until (`openUntil`; Infinity where it passes over).
ChScore.prototype._incomingSpans = function (measureElements, elementsById) {
  const indexOf = new Map(measureElements.map((element, index) => [element, index]));
  const incoming = new Map();
  const reach = (element, span) => {
    if (!incoming.has(element)) incoming.set(element, []);
    incoming.get(element).push(span);
  };
  const passOver = (fromIndex, toIndex, spanner) => {
    for (let index = fromIndex + 1; index < toIndex; index++) {
      reach(measureElements[index], { spanner: spanner, openUntil: Infinity });
    }
  };
  for (const [startIndex, measure] of measureElements.entries()) {
    for (const spanner of measure.querySelectorAll(':scope > [endid], :scope > [tstamp2]')) {
      const endEvent = elementsById.get(spanner.getAttribute('endid')?.substring(1));
      const endMeasure = endEvent?.closest('measure');
      if (endMeasure && endMeasure !== measure && indexOf.has(endMeasure)) {
        passOver(startIndex, indexOf.get(endMeasure), spanner);
        reach(endMeasure, { spanner: spanner, endEvent: endEvent });
      }
      // `<n>m+<beat>`: the span ends on that beat of the <measure> n on from this one
      const [, measuresOn, beat] =
        spanner.getAttribute('tstamp2')?.match(CH_TSTAMP2) ?? [];
      const target = measureElements[startIndex + Number(measuresOn)];
      if (Number(measuresOn) > 0 && target) {
        passOver(startIndex, startIndex + Number(measuresOn), spanner);
        reach(target, { spanner: spanner, openUntil: Number.parseFloat(beat) });
      }
    }
  }
  return incoming;
...
// A position in a measure built up from durations, rounded so the same beat reached two ways
// is the same number: a triplet's thirds are not exact in floating point, so 2 + ⅓ + ⅓ + ⅓ and
// 2 + 1 differ in the last place. Rounded where a position is used, never as it accumulates --
// rounding each third on the way compounds, and three of them no longer make one.
const chRoundTstamp = (tstamp) => Math.round(tstamp * 1e6) / 1e6;

// A @tstamp2: `<n>m+<beat>`, the beat of the <measure> n on from the one a span starts in
const CH_TSTAMP2 = /^(\d+)m\+([\d.]+)$/;

// Events a sub-measure is split between. mRest and mSpace stand for a whole measure, so they
// are re-cut to the piece they land in rather than moved into it.
const CH_SPLITTABLE_EVENTS = ['note', 'chord', 'rest', 'space', 'mRest', 'mSpace'];

// Where a barline can go inside a sub-measure, as tstamps from 1 -- more places for Verovio to
// wrap. Safe where every voice starts something, no beam, tuplet or spanner is open, and every
// lyric line starts a word; `relaxed` lets a slur, tie, hairpin or word be crossed too.
// `incoming` is what reaches in from an earlier <measure> (see _incomingSpans).
ChScore.prototype._measureSplitTstamps = function (measure, meter, incoming = [], relaxed = false) {
  // Slurs and ties are joined by @endid, which Verovio follows across a barline on its own; a
  // hairpin's @tstamp2 is rewritten once the splitting is done (see _finalizeMeasures)
  const isCrossable = (spanner) => relaxed && spanner.matches('slur, tie, hairpin');
  const [count, tstampUnit] = meter;

  // Walk each layer, recording where its events start, what is open across each of them, and
  // the syllables they carry. A beam or tuplet is open between its first and last event.
  const onsetsByLayer = [];
  const tstampById = new Map();
  const spanned = new Set(); // Tstamps a beam, tuplet or spanner is open across
  const sylTstampsByLine = new Map(); // Lyric line -> the tstamps it puts a syllable at
  const wordContinues = new Map(); // Lyric line -> tstamps whose syllable runs into the next
  const wordContinued = new Map(); // Lyric line -> tstamps whose syllable carries a word on
  for (const layer of measure.querySelectorAll('staff > layer')) {
    const onsets = new Map(); // Tstamp -> the event starting there
    let tstamp = 1;
    const walk = (container) => {
      const containerStart = tstamp;
      for (const element of container.children) {
        if (element.matches('beam, tuplet')) {
          walk(element);
          continue;
        }
        if (!CH_SPLITTABLE_EVENTS.includes(element.tagName)) continue;
        const at = chRoundTstamp(tstamp);
        onsets.set(at, element);
        tstampById.set(element, at);
        // A spanner can be attached to one note of a chord, which starts when the chord does
        for (const note of element.querySelectorAll('note')) tstampById.set(note, at);
        for (const syl of element.querySelectorAll('verse > syl')) {
          const line = syl.closest('verse').getAttribute('n');
          if (!sylTstampsByLine.has(line)) {
            sylTstampsByLine.set(line, new Set());
            wordContinues.set(line, new Set());
            wordContinued.set(line, new Set());
          }
          sylTstampsByLine.get(line).add(at);
          // @wordpos says where in its word a syllable falls: initial and medial both have
          // another syllable still to come, and medial and terminal one before them -- which
          // may be in an earlier <measure>
          const wordpos = syl.getAttribute('wordpos');
          if (['i', 'm'].includes(wordpos)) wordContinues.get(line).add(at);
          if (['m', 't'].includes(wordpos)) wordContinued.get(line).add(at);
        }
        tstamp += element.matches('mRest, mSpace')
          ? count : this._meiDurationTstamps(element, tstampUnit);
      }
      if (container.matches('beam, tuplet')) {
        for (const inner of onsets.keys()) {
          if (inner > chRoundTstamp(containerStart) && inner < chRoundTstamp(tstamp)) {
            spanned.add(inner);
          }
        }
      }
    ...
    onsetsByLayer.push({
      onsets: onsets,
      end: chRoundTstamp(tstamp),
      // A voice resting the whole measure can be cut anywhere
      isWholeRest: [...onsets.values()].some(event => event.matches('mRest, mSpace')),
    });
  }

  // A spanner is open from where it starts to where it ends: after its start event or @tstamp,
  // and up to and including its end event, or up to its @tstamp2. One that ends in a later
  // <measure> reaches past this one, so everything after its start is spoken for.
  const onsets = [...new Set(onsetsByLayer.flatMap(layer => [...layer.onsets.keys()]))];
  const eventTstamp = (event) =>
    tstampById.get(event) ?? tstampById.get(event?.closest(CH_SPLITTABLE_EVENTS.join()));
  const block = (isOpen) => { for (const onset of onsets) if (isOpen(onset)) spanned.add(onset); };
  for (const spanner of measure.querySelectorAll(':scope > [endid], :scope > [tstamp2]')) {
    if (isCrossable(spanner)) continue;
    const eventOf = (reference) => reference
      && measure.querySelector(`[*|id="${reference.substring(1)}"]`);
    const start = Number.parseFloat(spanner.getAttribute('tstamp'))
      || eventTstamp(eventOf(spanner.getAttribute('startid')));
    if (!start) continue;
    const [, measuresOn, beat] =
      spanner.getAttribute('tstamp2')?.match(CH_TSTAMP2) ?? [];
    const endTstamp = spanner.hasAttribute('endid')
      ? eventTstamp(eventOf(spanner.getAttribute('endid'))) : null;
    if (endTstamp != null) block(onset => onset > start && onset <= endTstamp);
    else if (measuresOn === '0') block(onset => onset > start && onset < Number.parseFloat(beat));
    else block(onset => onset > start);
  }
  // What reaches in from an earlier <measure> is open from the start up to where it ends here
  for (const span of incoming) {
    if (isCrossable(span.spanner)) continue;
    const end = span.endEvent ? eventTstamp(span.endEvent) : null;
    if (span.endEvent) block(onset => end == null || onset <= end);
    else block(onset => onset < span.openUntil);
  }

  const splitTstamps = [];
  for (const candidate of onsets.sort((a, b) => a - b)) {
    if (candidate <= 1 || candidate >= 1 + count || spanned.has(candidate)) continue;
    // Every voice has to break here too: one still sounding would have to be cut and tied
    const everyLayerBreaks = onsetsByLayer.every(layer =>
      layer.onsets.has(candidate) || layer.end <= candidate || layer.isWholeRest);
    if (!everyLayerBreaks) continue;
    // Every lyric line has to start a new word here -- read from the syllable here, which
    // knows about a word begun in an earlier <measure>, and from the one before it
    const everyLineStartsAWord = relaxed || [...sylTstampsByLine].every(([line, tstamps]) => {
      if (!tstamps.has(candidate) || wordContinued.get(line).has(candidate)) return false;
      const previous = Math.max(...[...tstamps].filter(tstamp => tstamp < candidate), -1);
      return previous < 0 || !wordContinues.get(line).has(previous);
    });
    if (everyLineStartsAWord) splitTstamps.push(candidate);
  }
  return splitTstamps;
}

// Split one <measure> in two at `splitTstamp`, returning the new second piece. The element
// itself stays as the first piece, keeping its id and its left barline, and runs into the new
// one invisibly; the records, not the document, say they are one measure.
ChScore.prototype._splitSubMeasure = function (measure, splitTstamp, meter, elementsById) {
  const meiParsed = this._scoreData.meiParsed;
  const [count, tstampUnit] = meter;
  // Anything at or past the split is the piece's -- an end marker too, written one tstamp past
  // the last beat
  const inPiece = (tstamp) => chRoundTstamp(tstamp) >= splitTstamp;
  // How long a beam or tuplet runs, in tstamps
  const containerTstamps = (container) => {
    let tstamps = 0;
    for (const element of container.children) {
      if (element.matches('beam, tuplet')) tstamps += containerTstamps(element);
      else if (CH_SPLITTABLE_EVENTS.includes(element.tagName)) {
        tstamps += this._meiDurationTstamps(element, tstampUnit);
      }
    }
    return tstamps;
  };
  const pieceId = (element) => `${element.getAttribute('xml:id')}-split1`;

  const piece = this._createMeiElement(meiParsed, 'measure');
  this._setMeiId(piece, pieceId(measure));
  measure.parentNode.insertBefore(piece, measure.nextSibling);

  // Deal the contents of every layer out to the side of the split each event falls on. Nothing
  // straddles a split -- that is what made it a split -- so events move whole, and a beam or
  // tuplet moves with its first event.
  for (const staff of Array.from(measure.querySelectorAll(':scope > staff'))) {
    const staffPiece = this._createMeiElement(meiParsed, 'staff');
    staffPiece.setAttribute('n', staff.getAttribute('n'));
    this._setMeiId(staffPiece, pieceId(staff));
    piece.appendChild(staffPiece);
    for (const layer of Array.from(staff.querySelectorAll(':scope > layer'))) {
      const layerPiece = this._createMeiElement(meiParsed, 'layer');
      layerPiece.setAttribute('n', layer.getAttribute('n') || '1');
      this._setMeiId(layerPiece, pieceId(layer));
      staffPiece.appendChild(layerPiece);
      let tstamp = 1;
      for (const element of Array.from(layer.children)) {
        // A whole-measure rest stands for whatever the measure holds, so each side gets its
        // own, cut to its own length, rather than the original moving to one of them
        if (element.matches('mRest, mSpace')) {
          const sides = [[layer, splitTstamp - 1], [layerPiece, 1 + count - splitTstamp]];
          sides.forEach(([target, length], side) => {
            const replacement = this._createMeiElement(
              meiParsed, element.matches('mRest') ? 'rest' : 'space');
            if (side === 0) {
              for (const attribute of Array.from(element.attributes)) {
                replacement.setAttributeNS(attribute.namespaceURI, attribute.name, attribute.value);
              }
            } else {
              this._setMeiId(replacement, pieceId(element));
            }
            const { dur, dots } = this._meiDurationFor(length, tstampUnit);
            replacement.setAttribute('dur', String(dur));
            if (dots > 0) replacement.setAttribute('dots', String(dots));
        ...
        if (inPiece(tstamp)) layerPiece.appendChild(element);
        if (element.matches('beam, tuplet')) {
          tstamp += containerTstamps(element);
        } else if (CH_SPLITTABLE_EVENTS.includes(element.tagName)) {
          tstamp += this._meiDurationTstamps(element, tstampUnit);
        }
      }
    }
  }

  // Notation attached to the measure goes with the moment it is attached to, its @tstamp
  // counted from the piece's own start. @tstamp2 is left to _finalizeMeasures, which rewrites
  // every one once all the splitting is done.
  for (const element of Array.from(measure.children)) {
    if (element.matches('staff')) continue;
    const startId = element.getAttribute('startid')?.substring(1);
    const tstamp = Number.parseFloat(element.getAttribute('tstamp'));
    const moves = startId
      ? elementsById.get(startId)?.closest('measure') === piece
      : Number.isFinite(tstamp) && inPiece(tstamp);
    if (!moves) continue;
    if (Number.isFinite(tstamp)) {
      element.setAttribute('tstamp', String(chRoundTstamp(tstamp - splitTstamp + 1)));
    }
    piece.appendChild(element);
  }

  // The measure's right barline moves to the piece, which now ends it. @metcon is left alone:
  // it marks a measure the engraver or a range cut short, which a split piece is not.
  const rightBarLine = measure.getAttribute('right');
  measure.setAttribute('right', 'invis');
  if (rightBarLine) piece.setAttribute('right', rightBarLine);
  return piece;
}

// Number every <measure> in the finished document the way _finalizeMeasures numbers the
// score's own -- an introduction and the passes an expansion made included -- and draw the
// barline between each two: double where one pass gives way to the next, hidden between two
// pieces of one measure.
ChScore.prototype._renumberMeasures = function (meiParsed) {
  // Which pass through the song a <measure> is in: the introduction, or one playthrough of an
  // expanded score -- the same for every <measure> of a score that is not expanded
  const passOf = (element) => (element.closest('section[type="introduction"]')
    ? 'introduction' : element.closest('[ch-iteration]')?.getAttribute('ch-iteration') ?? '');

  // Every <measure> here has a record, so which belong together is read off the records: two in
  // a row are one measure where they are consecutive pieces of one of the score's. A copy whose
  // source doesn't follow the one before it -- a range boundary -- opens a measure.
  const measures = [];
  let before = null;
  for (const element of meiParsed.querySelectorAll('measure')) {
    const subMeasure = this._scoreData.subMeasuresById[element.getAttribute('xml:id')];
    if (!subMeasure) return;
    const previous = measures.at(-1);
    const continues = previous != null
      && subMeasure.subMeasureIndex === previous.subMeasureIndex + 1
      && subMeasure.measureIndex === previous.measureIndex;
    // The barline before this <measure>: a double where one pass gives way to the next, and
    // none between two pieces of one measure -- unless the one written there already says more,
    // a repeat or a final bar. A pass change wins where a measure runs on across one.
    const written = before?.getAttribute('right') ?? null;
    if (before && passOf(before) !== passOf(element) && !element.hasAttribute('left')) {
      if ([null, 'single', 'invis'].includes(written)) before.setAttribute('right', 'dbl');
    } else if (continues && [null, 'single'].includes(written)) {
      before.setAttribute('right', 'invis');
    }
    before = element;
    if (continues) {
      previous.durationQ += subMeasure.durationQ;
      previous.subMeasureIndex = subMeasure.subMeasureIndex;
      previous.elements.push(element);
    } else {
      measures.push({
        timeSignature: [...subMeasure.timeSignature],
        durationQ: subMeasure.durationQ,
        measureIndex: subMeasure.measureIndex,
        subMeasureIndex: subMeasure.subMeasureIndex,
        elements: [element],
      });
    }
  }
  ...
  // The same rule the score's own measures are numbered by: a pickup the document opens on --
  // a first measure short of its meter -- is 0, and every measure after it takes the next number
  let counter = 0;
  measures.forEach((measure, index) => {
    const [count, unit] = measure.timeSignature;
    const opensOnPickup = index === 0 && measure.durationQ < count * (4 / unit);
    const measureNumber = opensOnPickup ? '0' : String(++counter);
    for (const element of measure.elements) element.setAttribute('n', measureNumber);
  });
}
...
ChScore.prototype._markHelpTextLyrics = function (lyricElementsByStaffAndLine) {
  ...
  this._rankVerseLines([...lyricElementsByStaffAndLine.values()].map(lyricElements => lyricElements[0]));
}
...
ChScore.prototype._rankVerseLines = function (lyricElements) {
  ...
  for (const lyricElement of lyricElements) {
    ...
    const row = Number.parseInt(lyricElement.getAttribute('n'));
    if (!Number.isNaN(row)) sungRows.add(row);
  }
...
// Number lyric lines by their order on the page, per stretch of music (an MEI section with its
// endings), and label the words every verse sings as lyric@name="chorus" would. Rewrites @n,
// which the lyric line ids are read off:
// - stacked melody lines move up together so the lowest is 1
// - a line never stacked beside ones that are, or one line singing alone for a while outside
...
ChScore.prototype._normalizeLyricLineNumbers = function () {
  ...
  let stretch = -1;
  let section = null;
  for (const measure of this._scoreData.meiParsed.querySelectorAll('measure')) {
    const container = measure.closest('ending, section');
    if (stretch === -1 || (container?.tagName !== 'ending' && container !== section)) {
      stretch += 1;
      section = container;
    }
  ...
  // stretch -> "staff.row" -> the line's elements and where it sings. Words on a part other than
  // the melody are a line of their own even on the melody's row ("It Is Well with My Soul"
  // engraves the parts' echo on verse 2's row), so they never count as a verse singing.
  const linesByStretch = new Map();
  for (const lyricElement of this._scoreData.meiParsed.querySelectorAll('verse')) {
    const key = stretchOf.get(lyricElement.closest('measure')) ?? -1;
    const staff = lyricElement.closest('staff')?.getAttribute('n') ?? '';
    const row = Number.parseInt(lyricElement.getAttribute('n'));
    if (Number.isNaN(row)) continue;
    const noteOrChord = lyricElement.closest('[ch-chord-position]');
    ...
    if (!lines.has(id)) {
      lines.set(id, { id, staff, row, help, secondary, elements: [], sungAt: new Set() });
    }
    const line = lines.get(id);
    line.elements.push(lyricElement);
    // Only sung syllables say where a line sings; a melisma's closing stub doesn't
    if (help || lyricElement.textContent.trim() === '') continue;
    const chordPosition = chordPositionOf.get(lyricElement);
    if (!Number.isNaN(chordPosition)) line.sungAt.add(chordPosition);
  }

  const isMelody = (line) => !line.help && !line.secondary && line.sungAt.size > 0;
  const overlaps = (line, other) => {
    for (const chordPosition of line.sungAt) if (other.sungAt.has(chordPosition)) return true;
    return false;
  };

  const rowOf = new Map();
  const shared = new Set();
  const setRow = (elements, row, isShared = false) => {
    for (const lyricElement of elements) {
      rowOf.set(lyricElement, row);
      if (isShared) shared.add(lyricElement);
    }
  };
  // The stretch before's melody lines, by engraved staff and row: the number each ended on, and
  // whether it ended labelled
  let carried = new Map();
  for (const key of [...linesByStretch.keys()].sort((a, b) => a - b)) {
    ...
    carried = new Map(melody.map(line => {
      const last = line.elements.at(-1);
      return [line.id, { row: rowOf.get(last) ?? line.row, shared: shared.has(last) }];
    }));
  }
  ...
  for (const [lyricElement, row] of rowOf) {
    ...
    // An engraving that names its lines calls them "verse" or "chorus", and can get it wrong
    // ("Far, Far Away on Judea's Plains" names its chorus a verse), so "verse" doesn't outrank
    // what the page shows; any other name does
  ...
  for (const lyricElement of moved) {
    ...
    for (const other of Array.from(lyricElement.parentElement?.children ?? [])) {
      if (other !== lyricElement && other.matches('verse') && other.textContent.trim() === ''
        && other.getAttribute('n') === lyricElement.getAttribute('n')) other.remove();
    }
  }
...
ChScore.prototype._normalizeLyricVerseNumbers = function (meiParsed, lyricElementsByStaffAndLine, staffNumberOf) {
  ...
  // Keyed "staff.n", the form the lyric line ids will take
  for (const [lyricLineId, lyricElements] of lyricElementsByStaffAndLine) {
    const lineNumber = this._verseLineNumber(lyricLineId);
...
ChScore.prototype._updateMei = function () {
  ...
  // The copies the last rebuild recorded went with it. What the score itself is written in
  // stays -- measures[].subMeasureIds names exactly those -- drawn as the score writes it, not
  // as the last rebuild did: a repeat sign an introduction adds belongs to that rebuild alone.
  const ownSubMeasureIds = new Set(
    this._scoreData.measures.flatMap(measure => measure.subMeasureIds));
  for (const subMeasureId of Object.keys(this._scoreData.subMeasuresById)) {
    if (!ownSubMeasureIds.has(subMeasureId)) delete this._scoreData.subMeasuresById[subMeasureId];
  }
  for (const element of this._scoreData.meiParsedComplete.querySelectorAll('measure')) {
    const subMeasure = this._scoreData.subMeasuresById[element.getAttribute('xml:id')];
    if (subMeasure) subMeasure.rightBarLine = element.getAttribute('right') ?? 'single';
  }
    ...
    if (this._currentOptions.expandScore === 'full-score' && this._scoreData.features.hasExpansion) {
      ...
      for (const sectionId of sectionIds) {
        ...
        for (const element of sectionsById[sectionId]) {
          ...
          const suffix = `-rend${sectionIdCounter[sectionId]}`;
          const copies = Array.from(newElement.querySelectorAll('measure'));
          const sourceIds = copies.map(measure => measure.getAttribute('xml:id'));
          this._suffixIds(newElement, suffix);
          // An exact copy, so each measure stands for the sub-measure it was cloned from
          copies.forEach((measure, index) => this._addRebuiltSubMeasure(measure, sourceIds[index]));
...
//           if (rightBarline != 'invis') measure.setAttribute('right', 'dbl');
      ...
      const melodyStaffNumbers = new Set();
      for (const el of this._scoreData.meiParsed.querySelectorAll('verse[ch-lyric-line-id]:not([ch-secondary])')) {
        melodyLineIds.add(el.getAttribute('ch-lyric-line-id'));
        melodyStaffNumbers.add(el.closest('staff')?.getAttribute('n'));
      }
      const secondaryRowByLineId = new Map();
      const secondaryLinesByStaff = new Map();
      for (const el of this._scoreData.meiParsed.querySelectorAll('verse[ch-lyric-line-id][ch-secondary]')) {
        const lineId = el.getAttribute('ch-lyric-line-id');
        if (melodyLineIds.has(lineId)) continue;
      ...
      for (const [ecpCounter, expandedChordPositionInfo] of this._scoreData.expandedChordPositions.entries()) {
        ...
        if (currentSectionChordPositions == null || !currentSectionChordPositions.has(chordPosition) || elements?.[0]?.hasAttribute('ch-expanded-chord-position')) {
          ...
          // One element per pass the expansion plays, and a caller's sections can claim more
          // passes than the score has: a two-part song's verses sung together is a section
          // whether or not the engraving repeats anything for it. The last section keeps the
          // rest rather than running off the end and taking the redraw with it.
          if (currentSectionIndex + 1 >= sectionElements.length) break;
          currentSectionIndex++;
        ...
        for (const element of elements) {
          ...
          if (lyricElements.length > 0 && !isIntroduction) {
            ...
            // Secondary lyrics are another voice's words, so match within their own stack. Echoes
            // on the melody's staff are engraved one per verse, on its row ("Stand by Me"), and
            // only this pass's is kept; a voice on a staff of its own keeps every line ("Far,
            ...
            const secondarySounding = secondaryElements.length > 1
              && melodyStaffNumbers.has(element.closest('staff')?.getAttribute('n'))
  ...
  for (const element of this._scoreData.meiParsed.querySelectorAll(definitions)) {
    ...
    const subMeasure = this._scoreData.subMeasuresById[element.getAttribute('xml:id')];
    if (!subMeasure) continue;
    const [count, unit] = subMeasure.timeSignature;
    const keyChanged = subMeasure.keySignatureId != null
    ...
    if (keyChanged) {
      const key = Object.values(this._keySignatures)
        .find(keys => keys[subMeasure.keySignatureId])[subMeasure.keySignatureId];
      const keySig = this._createMeiElement(this._scoreData.meiParsed, 'keySig');
      keySig.setAttribute('sig', key.meiSig);
      keySig.setAttribute('mode', key.tonality);
      scoreDef.appendChild(keySig);
    }
    if (meterChanged) {
      const meterSig = this._createMeiElement(this._scoreData.meiParsed, 'meterSig');
      meterSig.setAttribute('count', String(count));
  ...
  // and the song they are drawn from -- the way the score data counts measures, so @n reads
  // the same however the score is expanded, and settle the barlines between its measures
  this._renumberMeasures(this._scoreData.meiParsed);

  // The records follow the document as drawn: each <measure> with the barline settled above,
  // and none kept for a copy this rebuild made and then dropped, as a hidden section's is
  const drawnSubMeasureIds = new Set();
  for (const element of this._scoreData.meiParsed.querySelectorAll('measure')) {
    const subMeasureId = element.getAttribute('xml:id');
    drawnSubMeasureIds.add(subMeasureId);
    const subMeasure = this._scoreData.subMeasuresById[subMeasureId];
    if (subMeasure) subMeasure.rightBarLine = element.getAttribute('right') ?? 'single';
  }
  for (const subMeasureId of Object.keys(this._scoreData.subMeasuresById)) {
    if (!ownSubMeasureIds.has(subMeasureId) && !drawnSubMeasureIds.has(subMeasureId)) {
      delete this._scoreData.subMeasuresById[subMeasureId];
    }
  }
...
ChScore.prototype._updateSvg = function (svg) {
  ...
  for (const system of systems) {
    ...
    for (const measure of measures) {
      ...
      // An expanded score's copies each have a record, pointing at the measure they copy
...
ChScore.prototype._drawScore = function () {
  ...
  for (const section of this._scoreData.sections) {
    ...
    const lyricLines = section.lyricsAnnotated.replace(/\||•|_|◠|◡/g, '').trim().split('\n');
...
// How long an event sounds, in tstamps -- units of the meter's denominator, so a full measure
// is `count` of them whatever the meter: in 4/4 a tstamp is a quarter note, in 3/2 a half note.
ChScore.prototype._meiDurationTstamps = function (element, tstampUnit = 4) {
  // A grace note is written with a @dur but takes no time: it borrows from the note it leads to
  if (element.hasAttribute('grace')) return 0;
  const dur = Number.parseFloat(element.getAttribute('dur') || '4');
  const dots = Number.parseInt(element.getAttribute('dots') || '0');
  let durTstamps = tstampUnit / dur;
  for (let dot = 0; dot < dots; dot++) durTstamps += durTstamps / 2;
  return durTstamps * this._meiTupletRatio(element);
}

// How much of its written length an event inside a tuplet sounds for: a triplet fits three
// eighths into the time of two, so each sounds for 2/3 of one. Nested tuplets multiply.
ChScore.prototype._meiTupletRatio = function (element) {
  let ratio = 1;
  for (let ancestor = element.parentElement;
    ...
    if (num && numbase) ratio *= numbase / num;
  }
  return ratio;
}

// The inverse: a length in tstamps as MEI writes one, @dur with @dots. A length no single
// note can spell comes back as the nearest one that can.
ChScore.prototype._meiDurationFor = function (tstamps, tstampUnit = 4) {
  ...
  for (const validDur of validDurs) {
    if (Math.abs(meiDur - validDur) < Math.abs(meiDur - nearest)) nearest = validDur;
  }
...
ChScore.prototype._extractPianoIntroduction = function (meiParsed) {
  ...
  // The meter a sub-measure is written in, as [count, unit], from its record -- which follows a
  // change written anywhere before it, not only one inside it -- and 4/4 where the score names
  // none. The unit is also the tstamp unit: a quarter note in 4/4, a half note in 3/2.
  const getMeter = (measure) => {
    const timeSignature =
      this._scoreData.subMeasuresById[measure.getAttribute('xml:id')]?.timeSignature;
    return timeSignature?.[1] ? timeSignature : [4, 4];
  };
  ...
  const clipElement = (elem, currentTstamp, elemEnd, startTstamp, endTstamp, idMap, tstampUnit) => {
    ...
    if (!(currentTstamp >= startTstamp && elemEnd <= endTstamp)) {
      ...
      // Written back as the tuplet writes it: the clipped length is what it sounds for
      const result = this._meiDurationFor(newDur / this._meiTupletRatio(elem), tstampUnit);
  ...
  const copyLayerRange = (layer, startTstamp, endTstamp, idMap, tstampUnit) => {
    ...
    const processContainer = (container, currentTstamp) => {
      ...
      for (const child of container.children) {
        if (child.matches('beam, tuplet')) {
          ...
          const durTstamps = this._meiDurationTstamps(child, tstampUnit);
          const elemEnd = containerTstamp + durTstamps;
          const [from, to] = [chRoundTstamp(containerTstamp), chRoundTstamp(elemEnd)];

          if (from < endTstamp && to > startTstamp) {
            children.push(clipElement(child, from, to, startTstamp, endTstamp, idMap, tstampUnit));
          }
    ...
    const processElement = (elem, currentTstamp) => {
      ...
      if (!MUSICAL_ELEMENTS.includes(elem.tagName)) {
        const at = chRoundTstamp(currentTstamp);
        if (startTstamp <= at && at < endTstamp) {
      ...
      const durTstamps = this._meiDurationTstamps(elem, tstampUnit);
      const elemEnd = currentTstamp + durTstamps;
      const [from, to] = [chRoundTstamp(currentTstamp), chRoundTstamp(elemEnd)];

      if (from < endTstamp && to > startTstamp) {
        newLayer.appendChild(
          clipElement(elem, from, to, startTstamp, endTstamp, idMap, tstampUnit));
        hasContent = true;
      }
  ...
  // How long a sub-measure is as written, in tstamps: the longest any staff runs, since a rest
  // says nothing, and a whole-measure rest fills the meter. Only for sub-measures as written: a
  // clipped copy keeps its resting staff's mRest.
  const getSubMeasureDuration = (measure) => {
    const [count, tstampUnit] = getMeter(measure);
    ...
    for (const staff of measure.querySelectorAll('staff')) {
      ...
      longestTstamps = Math.max(longestTstamps, chRoundTstamp(tstampsIn(layer)));
    }
  ...
  const extractRange = (measures, [startId, startTstamp], [endId, endTstamp], idMap) => {
    ...
    return selected.map((measure, i) => {
      ...
      // Its own id, so it has a record of its own (see _addRebuiltSubMeasure). Two ranges can
      // copy the same sub-measure -- one ending part-way through it, the next picking it up.
      let copyId = `${measure.getAttribute('xml:id')}-intro`;
      for (let copy = 2; usedCopyIds.has(copyId); copy++) {
        copyId = `${measure.getAttribute('xml:id')}-intro${copy}`;
      }
      usedCopyIds.add(copyId);
      this._setMeiId(newM, copyId);
      ...
      // The range clips the sub-measures at its edges, so the copy's own span is worked out
      // here, in the score's time, rather than measured off the notation again later
      let span = null;
      if (mStart != null || mEnd != null) {
        const quartersPerTstamp = 4 / tstampUnit;
        const fromTstamp = mStart ?? 1;
        const toTstamp = Math.min(mEnd ?? Infinity, getSubMeasureDuration(measure) + 1);
        span = { durationQ: (toTstamp - fromTstamp) * quartersPerTstamp };
        const sourceStartQ =
          this._scoreData.subMeasuresById[measure.getAttribute('xml:id')]?.startQ;
        if (sourceStartQ != null) {
          span.startQ = this._tstampToQstamp(fromTstamp, sourceStartQ, Infinity, tstampUnit);
          span.endQ = span.startQ + span.durationQ;
          const qstamps = this._chordPositionQstamps();
          const first = this._bisectLeft(qstamps, span.startQ);
          span.firstChordPosition = qstamps[first] < span.endQ ? first : null;
        }
      }
      this._addRebuiltSubMeasure(newM, measure.getAttribute('xml:id'), span);
...
ChScore.prototype._getKeySignatureIds = function (tonality = 'major') {
  ...
  for (const element of this._scoreData.meiParsed.querySelectorAll('keySig, scoreDef[key\\.sig], staffDef[key\\.sig]')) {
    const keySignatureId = this._keySignatureIdOf(element, tonality);
    if (keySignatureId && keySignatureId !== keySignatureIds.at(-1)) keySignatureIds.push(keySignatureId);
  }
  return keySignatureIds;
}

// The key an element names -- a keySig, or a scoreDef or staffDef carrying one as attributes --
// as its slug ("e-flat-major"), or null where it names none the table knows. The tonality is
// the element's own @mode unless the caller settles it.
ChScore.prototype._keySignatureIdOf = function (element, tonality = null) {
  ...
  if (meiSig == null && meiPname == null) return null;
  ...
  const mode = tonality ?? element.getAttribute('mode') ?? element.getAttribute('key.mode') ?? 'major';
  const entry = Object.entries(this._getKeySignatures(mode) ?? {})
    .find(([, info]) => info.meiSig === meiSig || info.meiPnameAccid === meiPnameAccid);
  return entry?.[0] ?? null;
}
...
ChScore.prototype._getInlineVerseNumbers = function (meiParsed) {
  ...
  const verseLabels = meiParsed.querySelectorAll('verse[ch-lyric-line-id] label');
...
ChScore.prototype._normalizeParts = function (chordPositionIndex) {
  ...
  if (this._scoreData.parts.length === 0) {
    const build = (partsTemplate) => this._buildPartsFromTemplate(
      partsTemplate, this._scoreData.staffNumbers,
      this._scoreData.numChordPositions, this._scoreData.features.hasLyrics,
      chordPositionIndex
    );
    // A supplied template naming a place this score hasn't got is ignored (null)
    this._scoreData.parts = (this._suppliedTemplates.parts ? build(this._suppliedTemplates.parts) : null)
      ?? build(this._derivePartsTemplate(chordPositionIndex));
  }
...
ChScore.prototype._sungChordPositions = function (subMeasureData, chordPositionIndex, staffNumber, layer, startSubMeasure, endSubMeasure, limit = Infinity) {
  ...
  for (let smi = startSubMeasure; smi < endSubMeasure && chordPositions.length < limit; smi++) {
    for (const element of subMeasureData.subMeasures[smi].flatMap(
      measureElement => Array.from(measureElement.querySelectorAll(selector)))) {
...
ChScore.prototype._singingLayersBySubMeasure = function (subMeasureData, staffNumber) {
  return subMeasureData.subMeasures.map(measureElements => {
    // Unioned across the measure: a layer that only sings in its second sub-measure is still
    // singing in the measure
    const layersWithNotes = new Set();
    const layersWithSyllables = new Set();
    for (const measureElement of measureElements) {
      ...
      for (const layer of staff?.querySelectorAll('layer') ?? []) {
        ...
        layersWithNotes.add(layerNumber);
        if (layer.querySelector('syl:not(:empty)')) layersWithSyllables.add(layerNumber);
      }
    }
    return {
      layersWithNotes: [...layersWithNotes], layersWithSyllables: [...layersWithSyllables],
    };
  });
}
...
ChScore.prototype._getStaffSubMeasureData = function () {
  // Indexed by sub-measure as engraved: CH_MIN_SEGMENT_SUB_MEASURES and CH_UNSINGABLE_SUB_MEASURES
  // are tuned to it (counted by measure, "It Is Well with My Soul" reads two parts it hasn't).
  // A split's pieces fold back into the <measure> they were cut from -- hence a list per entry.
  const subMeasures = [];
  for (const element of this._scoreData.meiParsed.querySelectorAll('measure')) {
    const subMeasure = this._scoreData.subMeasuresById[element.getAttribute('xml:id')];
    if (subMeasures.length > 0 && subMeasure?.splitFrom) {
      subMeasures.at(-1).push(element);
    } else {
      subMeasures.push([element]);
    }
  }
  ...
  for (let smi = 0; smi < subMeasures.length; smi++) {
    for (const staffData of byStaff) {
      // A voice can enter, drop out or start singing at a sub-measure boundary, so the whole
      // measure is read before any of it is judged
      const staffElements = subMeasures[smi]
        .map(element => element.querySelector(`staff[n="${staffData.staffNumber}"]`))
        .filter(Boolean);
      if (staffElements.length === 0) continue;
      const subMeasure = staffData.bySubMeasure[smi];

      let syllablePositions = 0;
      let lowerVoiceNotes = 0;
      let secondVoiceEvents = 0;
      let secondVoiceSings = false;
      let mostLayers = 0;
      for (const staff of staffElements) {
        const layers = Array.from(staff.querySelectorAll('layer'));
        this._countMeasureVoices(layers, subMeasure.voiceCounts);
        mostLayers = Math.max(mostLayers, layers.length);
        ...
        syllablePositions += counts.syllablePositions;
        lowerVoiceNotes += counts.lowerVoiceNotes;
        secondVoiceEvents += counts.secondVoiceEvents;
        ...
        secondVoiceSings = secondVoiceSings
          || (staff.querySelector('note') !== null && !counts.secondVoiceResting);
      }

      subMeasure.syllables += syllablePositions;
      subMeasure.secondVoice += secondVoiceEvents;
      subMeasure.sounds = subMeasure.sounds || secondVoiceSings;
      // Counted once for the measure, however many sub-measures it is written in
      if (syllablePositions > 0) {
        subMeasure.sung += 1;
        const short = lowerVoiceNotes < syllablePositions;
        ...
        if (short && mostLayers > 1) subMeasure.lowerVoiceCantSing += 1;
      }
    }
  }
...
ChScore.prototype._getPartsSegmentBoundaries = function (subMeasureData, wholeStaves, chordPositionIndex) {
  ...
  for (let smi = 1; smi < subMeasureData.subMeasures.length; smi++) {
    ...
    const marks = subMeasureData.subMeasures[smi].flatMap(measureElement => Array.from(
      measureElement.querySelectorAll('dir[place]:not([type="coda"], [type="tocoda"], [type="segno"], [type="dalsegno"], [type="dacapo"], [type="fine"])')));
    for (const dir of marks) {
      ...
      const chordPosition = indexedChordPosition(
        dir, dir.closest('measure').getAttribute('xml:id'));
...
ChScore.prototype._convertPartsToTemplate = function (parts, form = 'chord-position') {
  ...
  return segments.map(segment =>
    `${this._writeTemplatePosition(segment.chordPosition, form)}:${segment.template}`).join('; ');
}

ChScore.prototype._buildPartsFromTemplate = function (partsTemplate, staffNumbers, numChordPositions, hasLyrics, chordPositionIndex = null) {
  ...
  // A segment with no position of its own starts at the top of the song
  const partsTemplates = normalizedPartsTemplate.split(';')
    .map(segment => segment.includes(':') ? segment : `0:${segment}`);
  ...
  let syllables;
  const syllableIndex = () => syllables ??= this._templateSyllableIndex(chordPositionIndex?.byElementId);
  ...
  for (let vm = 0; vm < partsTemplates.length; vm++) {
    ...
    const chordPosition = this._resolveTemplatePosition(
      chordPositionStr, numChordPositions, { syllableIndex, qstamps: templateQstamps });
    if (chordPosition === null) {
      console.warn(`Ignoring parts template: "${chordPositionStr}" names a position this score hasn't got.`);
      return null;
    }
...
ChScore.prototype._wholeNotesOf = function (element) {
  ...
  if (!dur) return null;
  const dots = Number.parseInt(element.getAttribute('dots')) || 0;
  return (1 / dur) * (2 - Math.pow(2, -dots)) * this._meiTupletRatio(element);
}
...
// A sections template lists the sections in the order they're sung, each a section character
// followed by the ranges it covers: "I(0-25[2,3]); V(0-32[2,3]:2.1); C(32-65:below)".
// What it doesn't write down is inferred the same way at both ends by the helpers below,
// which is what lets a derived template rebuild the sections it came from.

const CH_SECTION_CHAR_TO_TYPE = {
  // Vocal sections
  'V': 'verse',
  'C': 'chorus',
  'B': 'bridge',
...
const CH_SECTION_TYPE_TO_CHAR = Object.fromEntries(
  Object.entries(CH_SECTION_CHAR_TO_TYPE).map(([char, type]) => [type, char]));
// Nobody sings these, so an unwritten lyric location means no lyric lines rather than all of
// them -- the same distinction _getIntroSectionFromChordPositions draws by hand
const CH_INSTRUMENTAL_SECTION_TYPES = ['introduction', 'interlude'];

// The number each section carries within its type, answered for the whole list at once
// because a chorus's depends on where the verses are. Verses count from 1; a chorus counts
// from 0 when the song opens with one, since an initial chorus belongs to no verse yet, and
// from 1 otherwise; every other type counts from 1.
ChScore.prototype._numberSections = function (sectionTypes) {
  const firstVerseIndex = sectionTypes.indexOf('verse');
  const counters = {};
  return sectionTypes.map((type, index) => {
    if (!(type in counters)) {
      counters[type] = type === 'chorus' && firstVerseIndex !== -1 && index < firstVerseIndex ? 0 : 1;
    }
    return counters[type]++;
  });
}

// What a section is keyed and called by, from its type and its number within that type.
// Only verses carry a marker, so only a verse's name shows its number.
ChScore.prototype._sectionIdentity = function (type, number) {
  const marker = type === 'verse' ? String(number) : null;
  return {
    // The first introduction stays unnumbered: 'introduction' is the id the hideSectionIds
    // option documents and matches against
    sectionId: type === 'introduction' && number === 1 ? 'introduction' : `${type}-${number}`,
    name: this._stanzaName({ type: type, marker: marker }),
    marker: marker,
  };
}

// What every generated section starts as: the part of it the score itself says. Its id,
// name and pause depend on the finished list rather than on the section alone, so they are
...
// Every chord position belongs to a section. Music a section stops short of -- accompaniment
// after the last sung note, or a gap between two sections -- is taken up by whatever ends
// where it begins. Where two sections end together (a two-part song) both take it up.
ChScore.prototype._fillSectionGaps = function (sections, numChordPositions) {
  const covered = new Array(numChordPositions).fill(false);
  const ranges = sections.flatMap(section => section.chordPositionRanges ?? [])
    .filter(range => range.start != null && range.end != null);
  for (const range of ranges) {
    for (let cp = Math.max(0, range.start); cp < Math.min(numChordPositions, range.end); cp++) {
      covered[cp] = true;
    }
  }

  // Walked left to right over the coverage as it was, so extending a range can't change
  // which run is looked at next
  let cp = 0;
  while (cp < numChordPositions) {
    if (covered[cp]) { cp += 1; continue; }
    let runEnd = cp;
    while (runEnd < numChordPositions && !covered[runEnd]) runEnd += 1;

    const endingHere = ranges.filter(range => range.end === cp);
    if (endingHere.length > 0) {
      for (const range of endingHere) range.end = runEnd;
    } else {
      // Nothing comes before it, so what follows takes it up instead
      for (const range of ranges.filter(range => range.start === runEnd)) range.start = cp;
    }
    cp = runEnd;
  }
}
...
ChScore.prototype._getPauseAfters = function (sections, hasIntroBrackets = null) {
  ...
  const lastElement = this._scoreData.meiParsed.querySelector(
    `[ch-chord-position="${this._scoreData.numChordPositions - 1}"]:is(chord, note, rest)`);
  const songEndsOnShortSungNote = !!lastElement && !lastElement.matches('rest')
    && !!lastElement.querySelector('verse')
    && !(Number.parseInt(lastElement.getAttribute('dur')) < 4);
...
// Write the score down as templates -- parts, sections and lyric line breaks, each in both
// forms -- whether or not the caller handed any in, and derived from what was actually built,
// so a supplied template comes back resolved against this engraving rather than echoed.
// Called once the sections and stanzas settle: 'Mb' needs the measure numbers the timemap
// walk fills in.
ChScore.prototype._reportTemplates = function () {
  ...
  this._scoreData.templates = {
    ...
    partsTemplateInput: supplied.parts,
...
// Build a sections list from a sections template string, the way _buildPartsFromTemplate
// builds parts from a parts template. Every piece but the section character is optional, and
// each range spells out only what differs from the defaults: the whole song, every staff,
// and every lyric line (or no lyric line, for the instrumental types). Null if a bound names
...
ChScore.prototype._buildSectionsFromTemplate = function (sectionsTemplate, staffNumbers, numChordPositions) {
  const normalizedSectionsTemplate = (
    ...
    .replaceAll('Chorus', 'C')
    .replaceAll('Bridge', 'B')
    .replaceAll('Introduction', 'I')
    .replaceAll('Interlude', 'N')
    .replaceAll('Section', 'S')
  );

  // A section is its character (defaulting to a plain section) plus any number of '(…)' ranges;
  // a range is 'start-end', '[staffNumbers]', and ':lyricLocation', each optional but
  // written in that order
  const sectionPattern = /^([A-Z]?)((?:\([^)]*\))*)$/;
  // A range bound is a position as any template writes one -- a chord position, or
  // `measure@beat` (see _parseTemplatePosition) -- so it is matched loosely and read there.
  // Either bound may be left out ('11@1-' runs to the end of the song, '-20' starts at its
  // beginning), which is how a range reaches an edge it cannot name a position for.
  const rangePattern = /^(?:([\w@.]*)-([\w@.]*))?(?:\[([^\]]*)\])?(?::(.*))?$/;

  // A bound as this score reads it: absent is the default, and one naming a measure or chord
  // position the score hasn't got is null
  const syllableIndex = () => this._templateSyllableIndex();
  const bound = (positionStr, fallback, lyricLineIds) => positionStr === undefined || positionStr === ''
    ? fallback
    : this._resolveTemplatePosition(positionStr, numChordPositions, { syllableIndex, lyricLineIds });

  const sections = [];
  for (const sectionTemplate of normalizedSectionsTemplate.split(';')) {
    if (!sectionTemplate) continue;
    const [, sectionChar, rangesStr] = sectionPattern.exec(sectionTemplate) ?? [];
    if (rangesStr === undefined) {
      console.warn(`Skipping unrecognized section in sections template: "${sectionTemplate}".`);
      continue;
    }
    if (sectionChar && !(sectionChar in CH_SECTION_CHAR_TO_TYPE)) {
      console.warn(`Unrecognized section character "${sectionChar}" in sections template; reading it as a plain section.`);
    }
    const type = CH_SECTION_CHAR_TO_TYPE[sectionChar] ?? 'section';

    // A section with no '(…)' at all covers the whole song, which is what an empty range
    // spells out anyway, so it reads as a single default range
    const rangeStrs = rangesStr.match(/\([^)]*\)/g)?.map(range => range.slice(1, -1)) ?? [''];
    const chordPositionRanges = [];
    let placement = null;
    for (const rangeStr of rangeStrs) {
      const [, startStr, endStr, staffNumbersStr, lyricLocation] = rangePattern.exec(rangeStr) ?? [];
      if (startStr === undefined && staffNumbersStr === undefined && lyricLocation === undefined && rangeStr !== '') {
        console.warn(`Skipping unrecognized chord position range in sections template: "(${rangeStr})".`);
        continue;
      }

      // Staves the caller didn't name aren't in the score, so they can't be part of the
      // section; naming none of the score's staves leaves the section on all of them
      let rangeStaffNumbers = staffNumbers;
      if (staffNumbersStr) {
        const named = staffNumbersStr.split(',')
          .map(staffNumber => Number.parseInt(staffNumber))
          .filter(staffNumber => staffNumbers.includes(staffNumber));
        if (named.length > 0) rangeStaffNumbers = named;
      }

      // null is every lyric line, as it is on a generated section; [] is none of them.
      // Placement follows what the location says: words written below the music put the
      // section below it, and 'none' is a section the score doesn't place at all.
      let lyricLineIds = CH_INSTRUMENTAL_SECTION_TYPES.includes(type) ? [] : null;
      if (lyricLocation === 'below') {
        lyricLineIds = [];
        placement ??= 'below';
      } else if (lyricLocation === 'none') {
        lyricLineIds = [];
      } else if (lyricLocation) {
        lyricLineIds = lyricLocation.split(',').filter(Boolean);
        placement ??= 'inline';
      }

      // A location standing on its own, with no chord positions and no staves under it, is
      // a section the score never plays -- a verse printed below the music. It has nowhere
      // to be, rather than being everywhere, so it contributes no range at all.
      if (startStr === undefined && staffNumbersStr === undefined
        && (lyricLocation === 'below' || lyricLocation === 'none')) {
        continue;
      }

      const start = bound(startStr, 0, lyricLineIds);
      const end = bound(endStr, numChordPositions, lyricLineIds);
      if (start === null || end === null) {
        console.warn(`Ignoring sections template: range "(${rangeStr})" names a position this score hasn't got.`);
        return null;
      }
      chordPositionRanges.push({
        start: start,
        end: end,
        staffNumbers: rangeStaffNumbers,
        lyricLineIds: lyricLineIds,
      });
    }

    sections.push(this._newSection({
      type: type,
      // Where nothing said otherwise, the ranges answer: a section with music of its own is
      // placed inline with it, and one with no music anywhere isn't placed in the score at all
      placement: placement ?? (chordPositionRanges.length > 0 ? 'inline' : 'none'),
      chordPositionRanges: chordPositionRanges,
    }));
  }

  // Identity and pause depend on the section's place in the finished list, so they're
  // answered once it exists
  const numbers = this._numberSections(sections.map(section => section.type));
  const pauseAfters = this._getPauseAfters(sections);
  for (const [index, section] of sections.entries()) {
    Object.assign(section, this._sectionIdentity(section.type, numbers[index]));
    section.pauseAfter = pauseAfters[index];
  }

  return sections;
}

// A range's bounds in the form the caller asked for. The end is exclusive, so a range running
// to the end of the song names a position past the last one, which has no measure of its own:
// the portable form writes that as nothing at all, since another engraving's count of chord
// positions is not this one's. The exact form keeps the number.
ChScore.prototype._writeTemplateRange = function (chordPositionRange, form) {
  const runsToEnd = chordPositionRange.end >= (this._scoreData.numChordPositions ?? Infinity);
  const end = !runsToEnd ? this._writeTemplatePosition(chordPositionRange.end, form)
    : (form === 'measure-beat' ? '' : String(chordPositionRange.end));
  return `${this._writeTemplatePosition(chordPositionRange.start, form)}-${end}`;
}

// Write a sections list back out as a sections template, the counterpart to
// _buildSectionsFromTemplate. Only what the defaults don't say is written, and "every staff"
// and "the whole song" come from the score rather than the sections.
ChScore.prototype._convertSectionsToTemplate = function (sections, form = 'chord-position') {
  const staffNumbers = this._scoreData.staffNumbers;
  const numChordPositions = this._scoreData.numChordPositions;
  const allStaffNumbers = staffNumbers.join(',');
  const sectionTemplates = [];
  for (const section of sections) {
    const sectionChar = CH_SECTION_TYPE_TO_CHAR[section.type] ?? 'U';

    // Where the words go is the section's, not the range's, so every range spells out the
    // same location; only a list of lyric lines can differ from one range to the next
    let sectionLyricLocation = null;
    if (section.placement === 'below') sectionLyricLocation = 'below';
    else if (section.placement === 'none') sectionLyricLocation = 'none';

    const rangeTemplates = [];
    for (const chordPositionRange of section.chordPositionRanges) {
      let rangeTemplate = '';
      // 'below' and 'none' standing alone mean a section with no music of its own, so a
      // section that has music spells its chord positions out even when they span the song
      if (chordPositionRange.start !== 0 || chordPositionRange.end !== numChordPositions
        || sectionLyricLocation !== null) {
        rangeTemplate += this._writeTemplateRange(chordPositionRange, form);
      }
      const rangeStaffNumbers = (chordPositionRange.staffNumbers ?? staffNumbers).join(',');
      if (rangeStaffNumbers !== allStaffNumbers) rangeTemplate += `[${rangeStaffNumbers}]`;

      let lyricLocation = sectionLyricLocation;
      if (lyricLocation === null) {
        const lyricLineIds = chordPositionRange.lyricLineIds;
        const isDefault = CH_INSTRUMENTAL_SECTION_TYPES.includes(section.type)
          ? (lyricLineIds != null && lyricLineIds.length === 0)
          : lyricLineIds == null;
        if (!isDefault) {
          lyricLocation = (lyricLineIds == null || lyricLineIds.length === 0)
            ? 'none' : lyricLineIds.join(',');
          // Same reason: a bare ':none' would read as a section with no music
          if (lyricLocation === 'none' && rangeTemplate === '') {
            rangeTemplate += this._writeTemplateRange(chordPositionRange, form);
          }
        }
      }
      if (lyricLocation !== null) rangeTemplate += `:${lyricLocation}`;
      rangeTemplates.push(`(${rangeTemplate})`);
    }

    // A section with no ranges of its own is one the score never plays -- a verse printed
    // below the music. An empty range would read as the whole song, so it writes its
    // location alone.
    if (rangeTemplates.length === 0 && sectionLyricLocation !== null) {
      rangeTemplates.push(`(:${sectionLyricLocation})`);
    }
    sectionTemplates.push(`${sectionChar}${rangeTemplates.join('')}`);
  }
  return sectionTemplates.join('; ');
}

ChScore.prototype._normalizeSections = function () {
  // Sections come from the caller, a sections template, or the score itself, and are then
  // finished the same way whichever it was. The numbered steps below say how; only the last
  // is skipped for the caller's own sections, which are theirs as written.

  // ---- 1. What the score says ----

  const hasPrebuiltSections = this._scoreData.sections.length > 0;
  ...
  // Held rather than stamped on the spot: a section isn't named until the whole list is
  // known, so the id to write on its words isn't settled yet either. Keyed by the section
  // object, which doesn't change when its id is filled in.
  const lyricElementsBySection = new Map();
  const nameLyricElements = (section, lyricStanza) => {
    if (!lyricElementsBySection.has(section)) lyricElementsBySection.set(section, []);
    const named = lyricElementsBySection.get(section);
    for (const lyricElement of lyricStanza.lyricElements ?? []) {
      this._scoreData.stanzaNamedLyricElements.add(lyricElement);
      named.push(lyricElement);
    }
  };

  // Sections supplied by the caller win; otherwise build them from a template, if one was
  // given. Unlike parts, there is no derived template to fall back on -- a score with no
  // sections and no template is left to the automatic generation below. A template's
  ...
  // played out, which is what the stanzas are numbered from.
  ...
  if (hasSimpleSections && !hasInitialChorus && !this._scoreData.lyricsText) {
    ...
    const refrainLineNumbers = lyricStanzas
      ...
      .map(lyricLineId => this._verseLineNumber(lyricLineId));
  ...
  if (otherSections.length === 0) {
    if (lyricStanzas.length > 0) {
      // One section per stanza, each taking the stanza's words with it
      for (const lyricStanza of lyricStanzas) {
        // The staves the stanza's own verse is sung from, which is what its marker answers
        ...
        otherSections.push(this._newSection({
          ...
          // Nothing of it is sung from the staff, so it can only be printed below the music
      ...
      // Nothing to tell one passage from another: the whole of what is sung is one section
      otherSections = [this._newSection({
        chordPositionRanges: lyricChordPositionRanges.map(({ start, end }) => ({
          start: start,
          end: end,
  ...
  for (let ls = 0; ls < lyricStanzas.length; ls++) {
    ...
    while (pi < otherSections.length && (!startsHere(pi) || otherSections[pi].lyricsAnnotated)) pi += 1;
    ...
    if (section?.type === lyricStanza.type && !section.lyricsAnnotated) {
      ...
      // element for this one (see _sectionChordPositionRanges).
      otherSections.push(this._newSection({ ...lyricStanza, placement: 'below' }));
      nameLyricElements(otherSections.at(-1), lyricStanza);
    } else {
  ...
  const foldedReferenceChorus = foldWords(referenceChorus?.lyricsText);

  const stanzaBlocks = this._stanzaTextBlocks();

  // What the staff already sings, folded, as two readings per section: the tune's words, and
  // the tune's words with the parts' woven in at the positions they sound. A page setting the
  // words for singers sets all of them -- "It Is Well with My Soul" in French prints the echo
  // its lower parts answer with -- and matching a block against the tune alone misses that.
  // Indexed only where there are printed blocks to match, which few scores have.
  const everyVoice = stanzaBlocks.length > 0
    ? this._everyVoiceByChordPosition(melodyLyricElements) : new Map();
  const withEchoes = (section) => {
    const pieces = [];
    let echoed = false;
    for (const range of section.chordPositionRanges ?? []) {
      const lines = new Set(range.lyricLineIds ?? []);
      for (let cp = range.start; cp < range.end; cp++) {
        for (const { lyricLineId, isMelody, text } of everyVoice.get(cp) ?? []) {
          // This section's own words, and whatever is sung under them
          if (isMelody && lines.size > 0 && !lines.has(lyricLineId)) continue;
          if (!isMelody) echoed = true;
          pieces.push(text);
        }
      }
    }
    return echoed ? foldWords(pieces.join(' ')) : null;
  };

  const sungWords = stanzaBlocks.length === 0 ? [] : otherSections
    .filter(section => section.placement !== 'below')
    .flatMap(section => [foldWords(section.lyricsText), withEchoes(section)])
    .filter(Boolean)
    // Longest first, so a stanza that opens with another's words doesn't consume it
    .sort((a, b) => b.length - a.length);
  // Whether a printed block is nothing but sung stanzas, back to back: one block can print
  // several of them, and not in the order they are sung. A block can also be part of one
  // sung section: a page printing its refrain on its own, where the staff sings it as the
  // end of a verse ("I Have Faith in the Lord Jesus Christ" in French).
  const alreadySung = (text) => {
    let rest = foldWords(text);
    if (!rest) return false;
    if (sungWords.some(words => words.includes(rest))) return true;
    while (rest) {
      const sung = sungWords.find(words => rest.startsWith(words));
      if (!sung) return false;
      rest = rest.slice(sung.length);
    }
    return true;
  };
  ...
  for (const block of stanzaBlocks) {
    ...
    if (lyrics) {
      printedLyrics = prefix ? `${prefix.trim()}\n${lyrics}` : lyrics;
      sectionMarker = marker;

      // Skip lyric elements that are already sung from the staff. A section that claims the
      // verse but carries no words is a caller's own declaration of it (a sections template's
      // 'below', which says the words are printed rather than sung) -- these printed words
      // are the ones it was waiting for, so fill it in rather than adding a second copy.
      const declared = otherSections.find(section => this._cleanMarker(section.marker) === marker);
      if (declared) {
        if (!declared.lyricsText) {
          declared.lyricsText = printedLyrics;
          declared.lyricsAnnotated = printedLyrics;
        }
        continue;
      }
      // The verse it numbers may be sung from the staff without the score numbering it
      // there, which leaves nothing to match the marker against
      if (alreadySung(printedLyrics)) continue;
    } else {
    ...
    otherSections.push(this._newSection({
      ...
      lyricsText: printedLyrics,
      lyricsAnnotated: printedLyrics,
    }));
    ...
    if (referenceChorus && !foldWords(printedLyrics).endsWith(foldedReferenceChorus)) {
      // Wordless: the pass below gives every below chorus the sung chorus's words, so the
      // rule lives in one place
      otherSections.push(this._newSection({ type: 'chorus', placement: 'below' }));
    }
  }

  // A below chorus is printed rather than sung, so it has no syllables of its own to mark --
  // the sung chorus's markers name chord positions this copy doesn't own -- and it says what
  // the sung chorus says. Covers the copies added beside a printed verse just above and the
  // ones a caller's template declared, which record where a section is, not what it says.
  // Left empty the section is dropped, and a song printing its last verses under the music
  // loses their choruses ("School Thy Feelings").
  if (referenceChorus) {
    for (const section of otherSections) {
      if (section.placement !== 'below' || section.type !== 'chorus' || section.lyricsText) continue;
      section.lyricsText = referenceChorus.lyricsText;
      section.lyricsAnnotated = referenceChorus.lyricsText;
    }
  }

  // A song with one stanza has nothing for a chorus to alternate with, so sung words with
  // nothing else said about them are a verse; only the type is settled here, the identity
  // pass below names it. Counted over printed stanzas as well as sung, and never applied to
  // sections a caller stated the types of.
  if (!hasPrebuiltSections && !hasTemplateSections
    && otherSections.length === 1 && otherSections[0].lyricsAnnotated) {
  ...
  // Number and name every section by the shared rules, in one place, whichever path built
  // it -- verses count from 1 and carry that number as their marker, a chorus counts from 0
  // when the song opens with one, and the id and name follow from the type and the number.
  // Sections the caller supplied keep their own.
  if (!hasPrebuiltSections) {
    for (const section of this._scoreData.sections) section.type ||= 'section';

    // Nothing is sung anywhere in the score, so the verse the simple-score generator always
    // reaches for is a guess about words that aren't there: the passage is a plain section.
    // Set rather than defaulted, since that verse crowds out the default above. A template's
    // types are its own, and the instrumental types (introduction, interlude) already fit.
    if (!hasTemplateSections && !this._scoreData.features.hasLyrics) {
      for (const section of this._scoreData.sections) {
        if (!CH_INSTRUMENTAL_SECTION_TYPES.includes(section.type)) section.type = 'section';
      }
    }

    // Music before anyone sings is the introduction, wherever the sections came from. Asked of
    // the earliest position ANY lyric line sings, not of the first stanza: lines can begin at
    // different places, and an introduction must not be carved over one of them.
    if (!hasTemplateSections && !this._scoreData.sections.some(section => section.type === 'introduction')) {
      const sungChordPositions = [...melodyLyricElements.byChordPosition.keys()];
      const firstSungChordPosition = sungChordPositions.length > 0 ? Math.min(...sungChordPositions) : 0;
      if (firstSungChordPosition > 0) {
        ...
        const introSectionFromChordPositions = this._getIntroSectionFromChordPositions(
          [[0, firstSungChordPosition]], this._scoreData.staffNumbers);
        if (introSectionFromChordPositions) {
          // The sections that were covering it hand that music over
          for (const section of this._scoreData.sections) {
            for (const chordPositionRange of section.chordPositionRanges ?? []) {
              if (chordPositionRange.start === 0 && chordPositionRange.end > firstSungChordPosition) {
                chordPositionRange.start = firstSungChordPosition;
              }
            }
          }
          this._scoreData.sections.unshift(introSectionFromChordPositions);
        }
      }
    }
    ...
    for (const section of this._scoreData.sections) {
      const chordPositionRange = section.chordPositionRanges?.[0];
      if (!chordPositionRange || chordPositionRange.start === 0) continue;
      if (chordPositionRange.start >= firstMeasureEnd) continue;
      if (firstSounding < chordPositionRange.start) continue;
      chordPositionRange.start = 0;
    }

    // After the introduction above, which owns the music before the first word -- filling
    // gaps first would hand that music to whatever section happened to start next.
    // Before the pause rule below, which reads where a section ends.
    this._fillSectionGaps(this._scoreData.sections, this._scoreData.numChordPositions);
    // Before the numbering just below, so a verse it adds is named like the rest
    this._splitTwoPartFinalPass(lyricChordPositionRanges);
    // After the gap fill, which is what hands a section the trailing music in the first place,
    // and after the pass above, so a together pass it broke out is asked the same question.
    // Before the numbering, which is what names the interludes, and before the pause rule,
    // which reads where each section now ends.
    if (!hasTemplateSections) this._splitTrailingInterludes(this._scoreData.sections);
    ...
    for (const [index, section] of this._scoreData.sections.entries()) {
      ...
      if (usedSectionIds.has(identity.sectionId)) {
        let suffix = 2;
        while (usedSectionIds.has(`${identity.sectionId}-${suffix}`)) suffix += 1;
        identity.sectionId = `${identity.sectionId}-${suffix}`;
      }
      usedSectionIds.add(identity.sectionId);
      Object.assign(section, identity);
    }

    // And the pause, by the one rule as well: the paths that built these sections each
    // answered it their own way, or not at all
    const pauseAfters = this._getPauseAfters(this._scoreData.sections, hasIntroBrackets);
    for (const [index, section] of this._scoreData.sections.entries()) {
      section.pauseAfter = pauseAfters[index];
    }
  }

  // Now that the ids are settled, write them onto the words each section took
  for (const [section, lyricElements] of lyricElementsBySection) {
    for (const lyricElement of lyricElements) this._addSectionId(lyricElement, section.sectionId);
  }
...
// between a chorus and the verse after it ("Follow the Prophet"), or the ones a verse rests
// through ("Were You There?"). It runs from where the last word stops sounding, and is broken
// off only when a whole measure long and a section with music of its own follows.
ChScore.prototype._splitTrailingInterludes = function (sections) {
  // Nothing is sung anywhere in the score, so every section is already the music itself
  if (!this._scoreData.features.hasLyrics) return;
  // Music after the last section with any is how the song ends, not a passage into anything
  let lastWithMusic = -1;
  for (let i = sections.length - 1; i >= 0; i--) {
    if ((sections[i].chordPositionRanges ?? []).length > 0) { lastWithMusic = i; break; }
  }
  if (lastWithMusic < 1) return;
  ...
  for (const event of this._scoreData.meiParsed.querySelectorAll('staff :is(note, chord, rest)')) {
    ...
    if (Number.isNaN(chordPosition)) continue;
    // The last syllable with words, not the first: several can share a note where a language
    // elides them, and it is the last one's @con that says what follows. Help text isn't sung.
    const sylsByLine = new Map();
    for (const lyricElement of event.children) {
      if (!lyricElement.matches('verse:not([ch-help-text])')) continue;
      sylsByLine.set(lyricElement.getAttribute('n'), [...lyricElement.querySelectorAll(
        'syl:not(:empty):not([ch-help-text])')].at(-1) ?? null);
    }
    if (held.size === 0 && sylsByLine.size === 0) continue;
    ...
    for (const [lineNumber, syl] of sylsByLine) {
      if (!syl) continue;
      sungThrough(chordPosition, throughQ);
      if (syl.getAttribute('con') === 'u') held.set(lineNumber, chordPosition);
    }
  }
  if (sungThroughQ.size === 0) return;

  const added = [];
  for (const [index, section] of sections.entries()) {
    if (index >= lastWithMusic) continue;
    const ranges = section.chordPositionRanges ?? [];
    if (ranges.length === 0) continue;
    // A section that is already the music itself has no words to trail off from, and its
    // ranges can cover music a verse sings elsewhere -- an introduction replayed under one
    if (CH_INSTRUMENTAL_SECTION_TYPES.includes(section.type)) continue;

    // Back from the section's end to the last word sung in it, in the order its ranges are
    // played, so a stretch running on over a range boundary is found whole
    let lastRange = -1;
    let lastSung = -1;
    for (let r = ranges.length - 1; r >= 0 && lastSung < 0; r--) {
      for (let cp = ranges[r].end - 1; cp >= ranges[r].start; cp--) {
        if (sungThroughQ.has(cp)) { lastRange = r; lastSung = cp; break; }
      }
    }
    // Nothing sung in it at all: the section is already the music itself
    if (lastSung < 0) continue;

    // The interlude opens where the last word stops sounding, which is not the next chord
    // position: the accompaniment goes on striking chords under a note still being held.
    // Music in a later range is somewhere else in the score, so the note is over by then.
    const stopsQ = sungThroughQ.get(lastSung);
    let splitIndex = -1;
    let start = -1;
    for (let cp = lastSung + 1; cp < ranges[lastRange].end; cp++) {
      if (chordPositions[cp].startQ >= stopsQ) { splitIndex = lastRange; start = cp; break; }
    }
    for (let r = lastRange + 1; splitIndex < 0 && r < ranges.length; r++) {
      if (ranges[r].end > ranges[r].start) { splitIndex = r; start = ranges[r].start; }
    }
    if (splitIndex < 0) continue;

    // A whole bar of it at least, measured in the time signature rather than in the measure as
    // engraved: the short bar a song with a pickup ends on is one note held, not a passage
    const [beats, beatUnit] = this._scoreData
      .measures[chordPositions[start].measureIndex]?.timeSignature ?? [];
    if (!beats || !beatUnit) continue;
    let playedQ = 0;
    for (let r = splitIndex; r < ranges.length; r++) {
      const from = r === splitIndex ? start : ranges[r].start;
      if (ranges[r].end > from) {
        playedQ += chordPositions[ranges[r].end - 1].endQ - chordPositions[from].startQ;
      }
    }
    if (playedQ < beats * (4 / beatUnit)) continue;

    // Nothing is sung over any of it, so no range it takes names a lyric line
    const kept = ranges.slice(0, splitIndex);
    if (ranges[splitIndex].start < start) kept.push({ ...ranges[splitIndex], end: start });
    const moved = ranges.slice(splitIndex).map(range => ({ ...range, lyricLineIds: [] }));
    moved[0].start = start;

    section.chordPositionRanges = kept;
    added.push({ after: section, ranges: moved });
  }

  for (const { after, ranges } of added) {
    sections.splice(sections.indexOf(after) + 1, 0, this._newSection({
      type: 'interlude', placement: after.placement, chordPositionRanges: ranges,
    }));
  }
}

// A two-part song sings verse 1, then verse 2, then both parts together. That third pass is a
// verse of its own even though it sings no words the other two haven't, and it needs a section
// of its own to be written down and handed back; left as a second range on verse 2 it reads as
// that verse being sung twice. Only the sections move -- the stanzas stay as they are, which
// is what keeps the third pass out of the lyrics.
ChScore.prototype._splitTwoPartFinalPass = function (lyricChordPositionRanges = []) {
  if (!this._scoreData.features?.hasTwoPartMelody) return;
  ...
  for (const section of sections) {
    ...
    // Inside an earlier range of its own section, and the last thing that section does: a
    // repeat carrying on into another range is a real second pass (see
    // _getLyricChordPositionRanges), not the two parts coming together at the end.
    if (!ranges.some((earlier, e) => e < ranges.length - 1
      && last.start >= earlier.start && last.end <= earlier.end)) continue;

    ranges.pop();
    // Sung by everyone, so it names every lyric line the parts sing between them
    const lyricLineIds = [...new Set(sections.flatMap(other =>
      (other.chordPositionRanges ?? []).flatMap(range => range.lyricLineIds ?? [])))].sort();
    added.push({ after: section, range: { ...last, lyricLineIds: lyricLineIds } });
  }

  for (const { after, range } of added) {
    const section = this._newSection({
      type: after.type,
      placement: after.placement,
      chordPositionRanges: [range],
    });
    sections.splice(sections.indexOf(after) + 1, 0, section);
  }

  // The shapes below are alternatives, not both: a song whose together pass was found as a
  // repeated range is dealt with, and reading it a second way compounds anything the first
  // got wrong.
  if (added.length > 0) return;

  // The other shape the together pass takes: where it ends somewhere of its own -- a third
  // ending with words no other verse sings -- those words already give it a section, but only
  // over the ending, since the earlier verses' ranges swallowed the music it shares with them.
  // That shared stretch is prepended, naming every line again.
  //
  // A section with no type yet counts, since types are settled after this runs and this is the
  // passage the engraving says least about: "Love Is Spoken Here" prints "(3rd ending)" over
  // its closing words, which numbers nothing, so the stanza is left unclassified.
  const unclassified = (section) => !section.type || section.type === 'section';
  const verses = sections.filter(section =>
    (section.type === 'verse' || unclassified(section))
    && (section.chordPositionRanges?.length ?? 0) > 0);
  if (verses.length < 2) return;

  const firstRanges = verses.map(section => section.chordPositionRanges[0]);
  const linesSungInAll = [...new Set(firstRanges.flatMap(range => range.lyricLineIds ?? []))].sort();

  // Deriving the stanzas leaves neither shape to find: they come from the runs, which merge a
  // repeated body's passes into the one range spanning them, so no repeat is left to notice.
  // The passes are still in the ranges the lyric walk was handed, and a stretch played once
  // more often than there are verses to sing it is the parts singing them together -- which is
  // what makes the two paths agree on the same song.
  if (linesSungInAll.length > 1) {
    const playsByStretch = this._groupBy(lyricChordPositionRanges,
      range => `${range.start}-${range.end}`);
    for (const [stretch, plays] of playsByStretch) {
      if (plays.length !== verses.length + 1) continue;
      const [start, end] = stretch.split('-').map(Number);
      sections.push(this._newSection({
        type: verses.at(-1).type,
        placement: verses.at(-1).placement,
        chordPositionRanges: [{ ...firstRanges.at(-1), start: start, end: end,
          lyricLineIds: linesSungInAll }],
      }));
      return;
    }
  }
  ...
  const last = firstRanges.at(-1);
  const earlier = firstRanges.slice(0, -1);
  // What the earlier verses have in common: one piece of music, sung by each part in turn
  const shared = {
    start: Math.min(...earlier.map(range => range.start)),
    end: Math.min(...earlier.map(range => range.end)),
  };
  const lyricLineIds = [...new Set(earlier.flatMap(range => range.lyricLineIds ?? []))].sort();
  // Only where the last verse begins past all of it, and the parts really are separate lines
  if (shared.start < shared.end && last.start >= shared.end && lyricLineIds.length > 1) {
    const together = verses.at(-1);
    together.chordPositionRanges.unshift({ ...last, ...shared, lyricLineIds: lyricLineIds });
    // It is the verses before it sung at once, so it is one of them whatever the engraving
    // called it
    if (unclassified(together)) {
      together.type = verses.find(section => !unclassified(section))?.type ?? 'verse';
    }
  }
}
...
// Which stretches of the song carry lyrics, and what each one belongs to. Answered three
// ways: off the sections where there are any, off the expansion where the score says how
// many times it is played through, and otherwise as the whole song in a single range.
ChScore.prototype._getLyricChordPositionRanges = function (otherSections, melodyLyricElements) {
  ...
  if (otherSections.length > 0) {
    for (const [sectionIndex, sectionInfo] of otherSections.entries()) {
      for (let cpr = 0; cpr < sectionInfo.chordPositionRanges.length; cpr++) {
        ...
        if (twoPartFinalPass(chordPositionRange)) continue;
  ...
  return lyricChordPositionRanges;
}
...
ChScore.prototype._extendPlistForVerses = function (meiParsed, expansionIds, stackedLines, numChordPositions, melodyLyricElements = null) {
  ...
  // _verseSoundingAt sings the line whose number equals the visit count, and stacked lines are
  // numbered from 1 (_normalizeLyricLineNumbers), so a gap in the stack is still a pass.
  ...
  // How often a section must come round to sing all its lines: its highest, since each is sung
  // on the visit its number names
  const passesNeeded = (ref) => stackedLinesOf(ref).at(-1);
...
ChScore.prototype._getIntroSectionFromBrackets = function (meiParsed, staffNumbers) {
  ...
  return this._getIntroSectionFromChordPositions(introChordPositionRanges, staffNumbers);
}

ChScore.prototype._getIntroSectionFromChordPositions = function (introChordPositionRanges, staffNumbers) {
  let introSection;
  const chordPositionRanges = [];
  ...
  if (chordPositionRanges.length > 0) {
    introSection = this._newSection({
      type: 'introduction', chordPositionRanges: chordPositionRanges });
  }
  return introSection;
}
...
ChScore.prototype._generateSectionsFromSimpleScore = function (verseNumbers, hasInitialChorus, melodyLyricElements = this._melodyLyricElementIndex(), knownRefrainLineNumbers = []) {
  ...
  if (meiParsed.querySelector('verse:not([n="1"])')) {
    ...
    for (const chordPosition of melodyLyricElements.byChordPosition.keys()) {
      ...
      lineNumbersByCp[chordPosition] = lyricElements
        .map(lyricElement => this._verseLineNumber(lyricElement.getAttribute('ch-lyric-line-id')));
      lyricElementsByCp[chordPosition] = lyricElements;
    }
    // Runs of words every verse sings: labelled so by the engraving or by
    // _normalizeLyricLineNumbers, or in a section the expansion found to be the chorus the
    // song opens with ("All Things Bright and Beautiful")
    const chorusSectionChordPositions = new Set(Array.from(meiParsed.querySelectorAll('section[type="chorus"]'))
      .flatMap(section => section.getAttribute('ch-chord-position')?.trim().split(/\s+/) ?? []));
    for (const [chordPosition, lyricElements] of Object.entries(lyricElementsByCp)) {
      const shared = lyricElements.length > 0
        && (lyricElements.every(lyricElement => lyricElement.getAttribute('label') === 'chorus')
          || chorusSectionChordPositions.has(chordPosition));
      if (shared) {
    ...
    const chorusGaps = lyricGaps.filter(lyricGap =>
      this._isSharedRun(lyricGap.length, lyricElementsByCp[lyricGap[0]]?.[0])
      && !lineNumbersByCp[lyricGap[0]].some(lineNumber => refrainLineNumbers.has(lineNumber)));
    // A chorus is sung once where the verse is sung once, so shared words the verse sings its
    // way through are the verse's own refrain, and none of them is a chorus
    const isVerseRefrain = chorusGaps.some(lyricGap => this._isWithinVerse(
      Number.parseInt(lyricGap[0]), Number.parseInt(lyricGap.at(-1)), melodyLyricElements));
  ...
  for (const lyric of meiParsed.querySelectorAll('verse[ch-secondary]:not([ch-help-text])')) {
    const lineNumber = this._verseLineNumber(lyric.getAttribute('ch-lyric-line-id'));
  ...
  for (const lyricElement of meiParsed.querySelectorAll('verse[ch-pass][ch-lyric-line-id]')) {
    ...
    const claim = claims.get(pass)
      ?? { lineNumber: this._verseLineNumber(lyricElement.getAttribute('ch-lyric-line-id')), start: chordPosition, end: chordPosition + 1 };
  ...
  for (const lyricElement of meiParsed.querySelectorAll('verse[ch-lyric-line-id]:has(label)')) {
    const lineNumber = this._verseLineNumber(lyricElement.getAttribute('ch-lyric-line-id'));
  ...
  for (const staffNumber of this._scoreData.staffEntersAtVerse?.keys() ?? []) {
    for (const lyricElement of meiParsed.querySelectorAll(`staff[n="${staffNumber}"] verse[ch-lyric-line-id]`)) {
      const lineNumber = this._verseLineNumber(lyricElement.getAttribute('ch-lyric-line-id'));
      if (!Number.isNaN(lineNumber)) lateEntryLineNumbers.add(lineNumber);
    }
  }

  // The staff and verse pairings the score engraves, each naming its id (the row), since the
  // ranges below pair every singing staff with every verse. Help text isn't sung, so isn't one.
  const engravedLyricLineIds = new Map();
  for (const lyricElement of meiParsed.querySelectorAll('staff verse[ch-lyric-line-id]')) {
    const lyricLineId = lyricElement.getAttribute('ch-lyric-line-id');
    const lineNumber = this._verseLineNumber(lyricLineId);
    if (!Number.isNaN(lineNumber)) {
      engravedLyricLineIds.set(`${lyricLineId.split('.')[0]}.${lineNumber}`, lyricLineId);
    }
  }
  ...
  for (const verseNumber of verseNumbers) {
    ...
    const lyricLineIdsFor = (lineNumbers) => {
      ...
      for (const staffNumber of stavesPlaying) {
        for (const lineNumber of lineNumbers) {
          const id = engravedLyricLineIds.get(`${staffNumber}.${lineNumber}`);
          if (id) ids.push(id);
        }
      }
      return ids;
    ...
    for (const group of groups) {
      sections.push(this._newSection(group.isChorus
        ? { type: 'chorus', chordPositionRanges: group.ranges }
        // The marker is the score's own verse number, which the identity pass keeps
        : { type: 'verse', marker: group.marker, chordPositionRanges: group.ranges }));
    }
  }
...
ChScore.prototype._markSingleLineChordPositions = function (lyricChordPositionRanges, melodyLyricElements = this._melodyLyricElementIndex(), maxAllowedGap = this._maxLoneSyllables) {
...
// singers prints all of them, and matching a printed block against the tune alone misses a
// chorus whose parts echo it (see _normalizeSections).
ChScore.prototype._everyVoiceByChordPosition = function (melodyLyricElements) {
  const melody = new Set(melodyLyricElements?.lyricElements ?? []);
  ...
  for (const lyricElement of this._scoreData.meiParsed.querySelectorAll(
    ...
    const chordPosition = Number.parseInt(
      lyricElement.closest('note, chord')?.getAttribute('ch-chord-position'));
    if (Number.isNaN(chordPosition)) continue;
    const text = Array.from(lyricElement.querySelectorAll('syl:not([ch-help-text])'))
      .map(syl => syl.textContent).join('');
    if (!text.trim()) continue;
    if (!byChordPosition.has(chordPosition)) byChordPosition.set(chordPosition, []);
    byChordPosition.get(chordPosition).push({
      lyricLineId: lyricElement.getAttribute('ch-lyric-line-id'),
      isMelody: melody.has(lyricElement),
      text: text,
    });
  }
  return byChordPosition;
}

// The melody's verses, in one pass: the flat list, the same verses by chord position, and the
// lyric line numbers each section carries, built afresh on every call. Ask this rather than a
...
ChScore.prototype._melodyLyricElementIndex = function () {
  ...
  return { lyricElements, byChordPosition, linesBySection };
}
...
ChScore.prototype._lyricElementSoundingAt = function (lyricElements, passNumber, isSingleLine) {
  ...
  const namesAnotherVerse = () => this._markerNumber(sungElements[0].querySelector('label')?.textContent)
    !== this._verseLineNumber(sungElements[0].getAttribute('ch-lyric-line-id'));
  ...
  // A chorus alone at its chord position is everyone's words, whichever pass this is -- however
  // few syllables it runs to, which is what isSingleLine asks
  if (sungElements.length === 1 && sungElements[0].matches('[label="chorus"], [ch-chorus]')) {
    return soundingIndex(0);
  }
  return soundingIndex(sungElements
    .findIndex(ve => this._verseLineNumber(ve.getAttribute('ch-lyric-line-id')) === passNumber));
}
...
ChScore.prototype._gatherSyllables = function (lyricChordPositionRanges, ecpStart, melodyLyricElements = this._melodyLyricElementIndex()) {
  ...
  for (const { range, chordPosition: cp, expandedChordPosition: ecpCounter, passNumber }
    ...
    if (lyricElement) {
      ...
      const verseLabel = lyricElement.getAttribute('label');
      ...
      extractedLyricSyllables.push({
        ...
        verseLabel: verseLabel === 'chorus' ? null : verseLabel,
        isShared: verseLabel === 'chorus',
        ...
        // Whether this is a passage everyone sings, rather than one verse's own words:
        // read here, where the walk already knows it, for the chorus test below
  ...
  // Whether each syllable opens a word, read from the sequence rather than its own @wordpos,
  // which an engraving can contradict ('pre' initial, then 'pare' single). Tracked per lyric line
  // in sung order; a word left open carries into a line with none of its own, begun on a verse's
  // line and finished on the one every verse sings ("Be-" / "cause" in "Because God Loves Me").
  const wordPendingByLine = new Map();
  let previousLineId = null;
  for (const syllable of extractedLyricSyllables) {
    const syls = syllable.syls ?? [];
    if (syls.length === 0) continue;
    const lyricLineId = syllable.lyricLineIds?.[0] ?? '';
    const continuesAcrossLines = previousLineId !== null && previousLineId !== lyricLineId
      && wordPendingByLine.get(previousLineId) && !wordPendingByLine.has(lyricLineId);
    syllable.startsWord = !wordPendingByLine.get(lyricLineId) && !continuesAcrossLines;
    if (continuesAcrossLines) wordPendingByLine.set(previousLineId, false);
    wordPendingByLine.set(lyricLineId, ['i', 'm'].includes(syls.at(-1).wordpos));
    previousLineId = lyricLineId;
  }
...
ChScore.prototype._alignSyllablesToLyrics = function (expandedLyrics, syllables, staffNumbers) {
  ...
  // Where every line inside a stanza begins. The supplied text's own line division is kept
  // verbatim, so these are the line breaks of the answer -- and reading back which chord
  // position each one lands on is what turns hand-checked lyrics into a lyricLinesTemplate.
  // A blank line opens a stanza, not a line, so it is not one of these.
  const lineStartOffsets = [];
  for (let at = expandedLyrics.indexOf('\n'); at !== -1; at = expandedLyrics.indexOf('\n', at + 1)) {
    if (expandedLyrics[at + 1] !== '\n' && expandedLyrics[at - 1] !== '\n') lineStartOffsets.push(at + 1);
  }
  let nextLineStart = 0;
  ...
  // What the fold drops from the front of a syllable ("’" in "’ry"), memoized for the same
  // reason: the syllables come round again on every verse
  const droppedPrefixes = new Map();
  const droppedPrefixOf = (text) => {
    if (!droppedPrefixes.has(text)) {
      let dropped = 0;
      while (dropped < text.length && foldSyllable(text.slice(0, dropped + 1)) === '') dropped += 1;
      droppedPrefixes.set(text, text.slice(0, dropped));
    }
    return droppedPrefixes.get(text);
  };
  ...
  const claimForCurrentStanza = (syllable) => {
    ...
    // whichever way its words were arrived at
  ...
  for (const syllable of syllables) {
    ...
    // Process the match
    if (matched) {
      let originalPos = posMap[matchPos] !== undefined ? posMap[matchPos] : expandedLyrics.length;

      // The match lands where the syllable's folded text begins, which can be inside the
      // syllable as engraved: "ev’rything" is cut "ev" + "’ry", and folding drops the
      // apostrophe. Back up over the part the fold dropped, but only where the lyrics spell
      // it the same way, so nothing is claimed that isn't this syllable's.
      const prefix = droppedPrefixOf(syllable.text ?? '');
      if (prefix && expandedLyrics.slice(originalPos - prefix.length, originalPos) === prefix) {
        originalPos -= prefix.length;
      }
      ...
      // The first syllable at or past a line's first character is the one that line opens
      // on, so that is the chord position the break is written down as. Walked forward once
      // overall, like the stanza breaks above, since the matches only ever move forward.
      const chordPosition = syllable.chordPositions[0];
      if (chordPosition != null) {
        for (const lyricLineId of syllable.lyricLineIds ?? ['']) {
          this._addToSetMap(singers, chordPosition, lyricLineId);
        }
      }
      while (nextLineStart < lineStartOffsets.length
        && lineStartOffsets[nextLineStart] <= originalPos) {
        nextLineStart++;
        if (chordPosition == null) continue;
        for (const lyricLineId of syllable.lyricLineIds ?? ['']) {
          this._addToSetMap(breakers, chordPosition, lyricLineId);
        }
      }

      insertions.push([originalPos, this._syllableMarker(syllable)]);
      claimForCurrentStanza(syllable);
      pos = matchPos + normSylText.length;
    }

    // A syllable the match couldn't place is still sung, and still sung *here*: the walk is in
    // sung order. Left out, its chord positions join no stanza at all and the stanza covers a
    // fraction of the music it is sung to, while the words still look right. The match decides
    // which *words* these are, not which stanza sings them.
    if (!matched) claimForCurrentStanza(syllable);
  }
  ...
  // The words as given, before any marker is spliced into them
  const plainStanzasText = expandedLyrics.split('\n\n');
  ...
  for (let sz = 0; sz < stanzas.length; sz++) {
    stanzas[sz].lyricsText = this._applyFindReplace((plainStanzasText[sz] ?? '').trim());
    stanzas[sz].lyricsAnnotated = this._applyFindReplace(stanzasText[sz].trim());
  }
...
// Whether the verse's music carries on around a passage every verse sings -- verse words before
// it in its first syllable's section and after it in its last's -- making it the verse's refrain
// ("When I Go to Church") rather than a chorus. A chorus with a section of its own has no verse
// after it there, even where its pickup ends the verse's section ("Gethsemane").
ChScore.prototype._isWithinVerse = function (firstChordPosition, lastChordPosition, melodyLyricElements) {
  const byChordPosition = melodyLyricElements.byChordPosition;
  const first = byChordPosition.get(firstChordPosition)?.[0]?.closest('ending, section');
  const last = byChordPosition.get(lastChordPosition)?.[0]?.closest('ending, section');
  if (!first || !last) return false;
  const hasVerseIn = (lyricElements, section) => lyricElements.some(lyricElement =>
    lyricElement.getAttribute('label') !== 'chorus' && lyricElement.closest('ending, section') === section);
  let before = false;
  let after = false;
  for (const [chordPosition, lyricElements] of byChordPosition) {
    if (!before && chordPosition < firstChordPosition) before = hasVerseIn(lyricElements, first);
    if (!after && chordPosition > lastChordPosition) after = hasVerseIn(lyricElements, last);
    if (before && after) return true;
  }
  return false;
}

// A chorus the engraving labels, but which the verse sings its way through and out of again, is
// that verse's own refrain, not a chorus ("When I Go to Church"). Marked on the syllables because
// a complex-sections score's sections come from the stanzas, so nothing asks before the runs
// are cut; _generateSectionsFromSimpleScore asks _isWithinVerse the same way.
ChScore.prototype._markMidVerseRefrains = function (syllables) {
  // Only where the types were inferred: a caller who hands in sections, or a template naming
  // them, has said what this passage is, and a chorus they named is a chorus however the verse
  // around it reads. Read as a refrain it is typed 'verse', which no longer matches the section
  // declared for it, and the pairing of stanzas to sections stops there.
  if ((this._scoreData?.sections?.length ?? 0) > 0) return;

  const typeOf = (syllable) => syllable.sectionType ?? (syllable.isShared ? 'chorus' : syllable.verseLabel ?? null);
  const lineOf = (syllable) => syllable.lyricLineIds?.[0] ?? null;
  // Built on first use: most scores have no labelled stretch to ask about
  let melodyLyricElements = null;
  ...
  for (let index = 1; index < stretches.length - 1; index++) {
    ...
    // Bracketed by the verse, which goes into it and comes back out
    if (before.type !== 'verse' || after.type !== 'verse') continue;
    melodyLyricElements ??= this._melodyLyricElementIndex();
    if (!this._isWithinVerse(stretch.syllables[0].chordPositions[0],
      stretch.syllables.at(-1).chordPositions.at(-1), melodyLyricElements)) continue;
    ...
    for (const syllable of stretch.syllables) syllable.refrainOfLine = before.line;
  }
}
...
ChScore.prototype._syllableStanzaRuns = function (syllables) {
  ...
  for (const syllable of syllables) {
    ...
    // A section says what it is; fall back to the lyric element's own label for scores walked
    // without sections to align to. Words every verse sings aren't typed: whether they're a
    // chorus or a refrain is left to the runs (_markSingleLineRunsAsChorus, the refrain tests).
    let type = syllable.sectionType ?? syllable.verseLabel ?? null;
    ...
    if (syllable.refrainOfLine) {
      lyricLineId = syllable.refrainOfLine;
      type = 'verse';
    }
    // Words every verse sings are a line of their own, whatever number they're engraved on
    const isShared = syllable.isShared && !syllable.refrainOfLine;
    ...
    const startsNewRun = !current
      ...
      // An engraved verse number, and music heard again, each start a stanza -- unless they are
    ...
    const changesLine = !startsNewRun && ((!sameSection && (!this._continuesLyricLine(current.lyricLineId, lyricLineId)
      || current.isShared !== isShared)) || current.type !== type);
    // A stanza never breaks inside a word: where the line changes partway through one (a verse's
    // "Be-" finished by the "cause" every verse sings), the break moves back to where it began
    const opensWord = this._syllableStartsWord(syllable);
    const wordStart = changesLine && !opensWord
      ? current.syllables.findLastIndex(earlier => this._syllableStartsWord(earlier)) : -1;
    if (startsNewRun || (changesLine && opensWord) || wordStart > 0) {
      const carried = wordStart > 0 ? current.syllables.splice(wordStart) : [];
      if (carried.length) current.lastChordPosition = current.syllables.at(-1).chordPositions.at(-1);
      current = { lyricLineId: lyricLineId, type: type, lastChordPosition: -1, syllables: carried,
        sectionIndex: syllable.sectionIndex ?? null, label: syllable.label ?? null, isShared: isShared };
      runs.push(current);
    }
...
// Whether a line may begin on this syllable: it has to open a word, and which syllables do is
// settled once in _gatherSyllables rather than read off each one's own @wordpos, which an
// engraving can contradict. Falls back to the marking for syllables built without that pass.
// (_startsWord below answers the same question of a lyric element, before syllables exist.)
ChScore.prototype._syllableStartsWord = function (syllable) {
  return syllable?.startsWord ?? !['m', 't'].includes(syllable?.syls?.[0]?.wordpos);
}
...
// How long one lyric line can sing alone, where the verses are stacked, and still be one verse's
// own words -- a syllable or two where the others hold a note. Any longer and every verse is
// singing it: a chorus, or words closing each verse. Read by _normalizeLyricLineNumbers, which
// labels those words, _generateSectionsFromSimpleScore and _markSingleLineChordPositions.
ChScore.prototype._maxLoneSyllables = 3;

// Whether one line singing alone for this many syllables is every verse's words. A capital
// opening the run's first word says a new phrase starts there, a second, weaker signal that
// lets a run exactly _maxLoneSyllables long count; a language that doesn't mark phrases with
// case is left on the threshold.
ChScore.prototype._isSharedRun = function (syllableCount, firstLyricElement) {
  if (syllableCount > this._maxLoneSyllables) return true;
  if (syllableCount < this._maxLoneSyllables || !firstLyricElement) return false;
  const syl = firstLyricElement.querySelector('syl:not(:empty)');
...
// Cut out a chorus by its words where the sections don't mark it ("Gethsemane" starts its chorus
// on a pickup in the verse's section and ends it in the next): a passage sung twice to the same
// words over the same notes, grown while they agree. Taken only where both copies are phrases
// longer than a lone line, part company where the music does, aren't the verse's own refrain,
// and leave a verse besides. Not where the sections came first.
ChScore.prototype._splitRunsAtRepeatedWords = function (runs, phraseStarts) {
  const syllables = runs.flatMap(run => run.syllables);
  if (syllables.some(syllable => syllable.sectionType != null)) return runs;
  const runOf = new Map();
  for (const run of runs) for (const syllable of run.syllables) runOf.set(syllable, run);
  const words = syllables.map(syllable => this._foldedSyllable(syllable.text));
  const chordPositionOf = (index) => syllables[index].chordPositions[0];
  // Built on first use: most scores have no passage to ask about
  let melodyLyricElements = null;
  // A phrase opens at a stanza's start, and wherever the segmenter put one
  const opensPhrase = (index) => index <= 0 || index >= syllables.length
    || runOf.get(syllables[index]) !== runOf.get(syllables[index - 1])
    || this._phraseStartsFor(phraseStarts, runOf.get(syllables[index])).has(chordPositionOf(index));

  const byChordPosition = new Map();
  syllables.forEach((syllable, index) => {
    const chordPosition = chordPositionOf(index);
    if (!byChordPosition.has(chordPosition)) byChordPosition.set(chordPosition, []);
    byChordPosition.get(chordPosition).push(index);
  });
  const cuts = new Set();
  ...
  for (const indices of byChordPosition.values()) {
    for (let a = 0; a < indices.length; a++) {
      for (let b = a + 1; b < indices.length; b++) {
        const [first, second] = [indices[a], indices[b]];
        if (seen.has(`${first}:${second}`) || !words[first] || words[first] !== words[second]) continue;
        let from = 0;
        while (first - from - 1 >= 0 && words[first - from - 1] && words[first - from - 1] === words[second - from - 1]) from++;
        let to = 0;
        while (first + to + 1 < second - from && second + to + 1 < syllables.length
          && words[first + to + 1] && words[first + to + 1] === words[second + to + 1]) to++;
        for (let k = -from; k <= to; k++) seen.add(`${first + k}:${second + k}`);
        // A chorus coming round parts company with itself where the music does, at the verses
        // either side. Words that change over the very notes they share are one passage sung
        // twice reworded ("Gethsemane" closes on "gave this gift" and then "gives"), not a return.
        const sameNotes = (x, y) => x >= 0 && y < syllables.length && chordPositionOf(x) === chordPositionOf(y);
        if (sameNotes(first - from - 1, second - from - 1) || sameNotes(first + to + 1, second + to + 1)) continue;
        // Words agreeing by chance before it ("could go." and "long ago.") are trimmed back to
        // where both copies open a phrase. Where they part company is where the chorus ends, so
        ...
        // pickup and closing word either side of it, which can sit in the verse's sections
        let sharedFrom = Infinity;
        let sharedTo = -Infinity;
        for (let k = start; k <= to; k++) {
          if (chordPositionOf(first + k) !== chordPositionOf(second + k)) continue;
          sharedFrom = Math.min(sharedFrom, chordPositionOf(first + k));
          sharedTo = Math.max(sharedTo, chordPositionOf(first + k));
        }
        melodyLyricElements ??= this._melodyLyricElementIndex();
  ...
  // A chorus is what the verses alternate with, so a song that is all one passage sung again
  // and again has none ("For Health and Strength")
  if (cuts.size === 0 || syllables.every(syllable => chorusSyllables.has(syllable))) return runs;
  ...
  for (const run of runs) {
    let pieceStart = 0;
    for (let index = 1; index <= run.syllables.length; index++) {
      if (index < run.syllables.length && !cuts.has(offset + index)) continue;
      const pieceSyllables = run.syllables.slice(pieceStart, index);
      const isChorus = pieceSyllables.every(syllable => chorusSyllables.has(syllable));
      split.push({ ...run, syllables: pieceSyllables,
        type: isChorus ? 'chorus' : run.type,
        lastChordPosition: pieceSyllables.at(-1).chordPositions.at(-1) });
      pieceStart = index;
    }
...
// Join a chorus the score sings twice in a row back into one stanza: engraved once
...
ChScore.prototype._mergeRepeatedChorusRuns = function (runs) {
  ...
  // Typed a chorus, or words labelled as every verse's that nothing has typed yet
  const isChorus = (run) => run.type === 'chorus' || (!run.type && run.isShared);

  // Sung over the same notes, starting where the other starts: the music played again
  const merged = [];
  for (const run of runs) {
    const previous = merged.at(-1);
    // To the same words, or reworded over the same music straight after, starting where it
    // starts ("Gethsemane" closes on its chorus twice, "gave this gift" and then "gives")
    if (previous && isChorus(run) && isChorus(previous)
      && ((previous.lyricLineId === run.lyricLineId && textOf(previous) === textOf(run))
        || previous.syllables[0]?.chordPositions[0] === run.syllables[0]?.chordPositions[0])) {
...
// A round's entry marks belong to the music rather than to one playthrough: every stanza opens
// at ➀ because every stanza is the same round begun again. The mark is engraved once, over the
// first pass's pickup, so a stanza whose own positions never come past it has nothing to read
// it from. Matched on the word the stanza opens with, so a stanza opening somewhere else is
// left alone; _movePickupSyllables covers the stanzas whose syllables it moves.
ChScore.prototype._carryRoundMarkersToRepeats = function (runs) {
  if (!this._scoreData.features?.hasRound) return;

  const markerByOpeningWord = new Map();
  for (const run of runs) {
    const first = run.syllables[0];
    if (!first || !this._syllableStartsWord(first)) continue;
    const opening = this._foldForMatching(first.text ?? '', 'remove');
    if (!opening) continue;
    if (first.roundMarker) {
      // The first stanza to open with this word is the one whose mark the rest read
      if (!markerByOpeningWord.has(opening)) markerByOpeningWord.set(opening, first.roundMarker);
    } else {
      first.roundMarker = markerByOpeningWord.get(opening) ?? null;
    }
  }
}

// Mark the runs everyone sings -- one lyric line where the verses are stacked, which is all
// a complex score says about its chorus. Not in a round or a two-part score, where stacked
// lines are voices singing at once. Marked rather than typed because the type takes part in
// how runs are split; _getLyricsFromSyllables types them once the stanzas are settled.
ChScore.prototype._markSingleLineRunsAsChorus = function (runs) {
  // Words labelled as every verse's that still stand as a stanza of their own, once a verse has
  // taken back the few closing it (_mergeSingleLineRuns), are a chorus -- stacked or not, and
  // whether the engraving labelled them or _normalizeLyricLineNumbers did. The refrain tests
  // after this can still read one as a refrain.
  for (const run of runs) {
    if (run.type || run.syllables.length === 0) continue;
    if (run.isShared) run.isChorus = true;
  }
  if (this._scoreData.features.hasTwoPartMelody || this._scoreData.features.hasRound) return;
  if (!this._scoreData.meiParsed.querySelector('verse:not([n="1"])')) return;
  // Something the verses sing to themselves has to be there for a chorus to alternate with
  if (!runs.some(run => run.syllables.some(syllable => !syllable.isSingleLine))) return;

  for (const run of runs) {
    if (run.type || run.syllables.length === 0) continue;
    if (run.syllables.every(syllable => syllable.isSingleLine)) run.isChorus = true;
  }
}
...
ChScore.prototype._mergeSingleLineRuns = function (runs, phraseStarts) {
  ...
  for (let r = 1; r < runs.length; r++) {
    ...
    // Words labelled as every verse's are a line of their own, however they're numbered
    const isSibling = !run.isShared && siblingLineIds.has(run.lyricLineId);
    if (!continuesWord && !assertsNothing && !isSibling) continue;
...
ChScore.prototype._getLyricsFromSyllables = function (syllables) {
  ...
  const sectionRuns = this._splitRunsAtRepeatedWords(
    this._splitRunsAtRepeatedSections(provisionalRuns, phraseStarts), phraseStarts);
  ...
  this._carryRoundMarkersToRepeats(runs);
  ...
  // exist. A caller's lyricLinesTemplate speaks here and not on that walk -- it says where a
  // stanza's lines divide, not where the stanzas are, and the passes between the two walks
  // read phrase starts to decide what is one stanza.
  const finalPhraseStarts = this._getPhraseStartsByStaff(syllables, runs, { applyTemplate: true });

  // Where the lines broke, for the template reported once the stanzas settle. Taken from
  // what each lyric line asked for rather than from emission, because that is the point a
  // supplied template replaces -- one derived here and handed back re-enters the same walk.
  // `singers` is who sang each position at all, which is what tells a break every verse
  // there takes from one only some of them do (see _lyricLineBreakEntries).
  const singers = new Map();
  for (const run of runs) {
    for (const syllable of run.syllables) {
      this._addToSetMap(singers, syllable.chordPositions[0], run.lyricLineId ?? '');
    }
  }
  const breakers = new Map();
  for (const [lyricLineId, asked] of finalPhraseStarts.askedByLine) {
    for (const chordPosition of asked) this._addToSetMap(breakers, chordPosition, lyricLineId);
  }
  this._scoreData.lyricLineBreaks = { breakers: breakers, singers: singers };

  // Pickups join the verse they lead into before the words are built, so a word split across
  // the two ("A-" in a first ending, "quí" after the repeat) is one word. Where they join is
  // not a line break: the pickup opens the verse's first line.
  const pickupJoins = new Set();
  const stanzaRuns = this._mergePickupRuns(runs, pickupJoins);
  ...
  for (const run of stanzaRuns) for (const [index, syllable] of run.syllables.entries()) {
    ...
    } else if (runPhraseStarts.has(chordPosition) && !pickupJoins.has(syllable)) {
      builder.breakLine();
    }
    ...
    const syls = syllable.syls ?? [];
    const marker = this._syllableMarker(syllable);
    // A dash held where another verse sings writes nothing; one elided onto a word ("hinaus –")
    // is the word's punctuation, and stays
    if (syls.length === 0) {
      builder.add(syllable.text, null, false, false, marker);
    } else if (!syls.every(syl => this._patterns.holdDash.test(syl.text.trim()))) {
...
ChScore.prototype._scorePhraseStarts = function (syllables, runs = null) {
  ...
  for (const run of runs) {
    ...
    for (let i = 1; i < run.syllables.length; i++) {
      ...
      if (!this._syllableStartsWord(syllable)) continue; // Never break mid-word
...
ChScore.prototype._addBeatPhaseBonus = function (scores, runs) {
  ...
  for (const run of runs) {
    ...
    if (opening == null || opening >= earliest) continue;
    earliest = opening;
    songPhraseBeat = beatOf(opening);
  }
  if (songPhraseBeat == null) return;
...
ChScore.prototype._foldedRunStream = function (run) {
  ...
  run.syllables.forEach((syllable, index) => {
    for (const char of this._foldedSyllable(syllable.text)) {
      stream += char;
...
// A syllable's text folded for matching. Memoized per text rather than per run: syllables repeat
// heavily across verses, and a run's stream would outlive its syllables, since later passes
// rebuild runs by spreading them.
ChScore.prototype._foldedSyllable = function (text) {
  const key = text ?? '';
  this._foldedSyllableText ??= new Map();
  if (!this._foldedSyllableText.has(key)) {
    this._foldedSyllableText.set(key, this._foldForMatching(key, 'remove'));
  }
  return this._foldedSyllableText.get(key);
}
...
ChScore.prototype._printedStanzaLines = function () {
  if (!this._printedLines) {
    // Wrapped blocks are left out: this is read for where a stanza divides, and a block
    // wrapped to the page divides where the column ended
    this._printedLines = this._stanzaTextBlocks().filter(block => !block.wrapped).map(block => {
      const lines = [];
...
ChScore.prototype._printedBreaksForRun = function (run) {
  ...
  for (const line of stanza.lines.slice(0, -1)) {
    ...
    const opensWord = this._syllableStartsWord(run.syllables[index]);
...
//
//   "12; 34; 56"          chord positions -- exact, and cheapest read back against the
...
// same places. An entry names the lyric lines it belongs to only where they disagree, the way
// a sectionsTemplate range names its staves.
ChScore.prototype._convertLyricLinesToTemplate = function (lyricLineBreaks, form = 'measure-beat') {
  return this._writeLyricLinesTemplate(this._lyricLineBreakEntries(lyricLineBreaks), form);
}

// Add a value to a Set kept under `key`, creating the Set on first use.
ChScore.prototype._addToSetMap = function (map, key, value) {
  if (!map.has(key)) map.set(key, new Set());
  map.get(key).add(value);
}

// The collected breaks as one entry per chord position, each naming the lyric lines that
// break there -- or null for "every line sung here does", which is what leaves the entry
// unqualified and portable.
ChScore.prototype._lyricLineBreakEntries = function (lyricLineBreaks) {
  if (!lyricLineBreaks) return null;
  const { breakers, singers } = lyricLineBreaks;
  return [...breakers.keys()].sort((a, b) => a - b).map(chordPosition => {
    const broke = breakers.get(chordPosition);
    const sang = singers.get(chordPosition) ?? broke;
    const allAgree = [...sang].every(lyricLineId => broke.has(lyricLineId));
    return {
      chordPosition: chordPosition,
      lyricLineIds: allAgree ? null : [...broke].sort(),
    };
  });
}

// Render entries in either form. A position with no measure behind it falls back to its
// chord position, which is still readable against this score.
ChScore.prototype._writeLyricLinesTemplate = function (entries, form = 'measure-beat') {
  if (!entries || entries.length === 0) return null;
  return entries.map(entry => {
    const text = this._writeTemplatePosition(entry.chordPosition, form);
    return entry.lyricLineIds ? `${text}[${entry.lyricLineIds.join(',')}]` : text;
  }).join('; ');
}

// Rewrite a template in the other form, against this score. The two are interchangeable --
// a caller reading a stored template back against the score it came from wants the chord
// positions, and one storing it wants the measures and beats.
ChScore.prototype._convertLyricLinesTemplateForm = function (lyricLinesTemplate, form) {
  return this._writeLyricLinesTemplate(this._parseLyricLinesTemplate(lyricLinesTemplate), form);
}

// A template's entries resolved against this score, or null if there is no template. Parsed
// per entry rather than per template, so a list mixing the two forms costs nothing to
// tolerate -- nothing here emits one. Cached, since every run asks the same question.
ChScore.prototype._parseLyricLinesTemplate = function (lyricLinesTemplate) {
  if (!lyricLinesTemplate) return null;
  const cache = this._lyricLinesTemplateCache;
  if (cache?.scoreData === this._scoreData && cache.template === lyricLinesTemplate) {
    return cache.entries;
  }

  // A position naming a place this score hasn't got says the template was written against
  // another engraving, so the whole of it is ignored (null), as if none had been given
  let entries = [];
  for (const entry of lyricLinesTemplate.split(';')) {
    const match = entry.trim().match(/^([^[\]]+?)(?:\[([^\]]*)\])?$/);
    if (!match) continue;
    const [, position, lyricLineIdsStr] = match;
    const lyricLineIds = lyricLineIdsStr ? lyricLineIdsStr.split(',').map(id => id.trim()) : null;
    const chordPosition = this._resolveTemplatePosition(position, this._scoreData.numChordPositions);
    if (chordPosition === null) {
      console.warn(`Ignoring lyric lines template: "${position.trim()}" names a position this score hasn't got.`);
      entries = null;
      break;
    }
    entries.push({ chordPosition: chordPosition, lyricLineIds: lyricLineIds });
  }
  entries?.sort((a, b) => a.chordPosition - b.chordPosition);

  this._lyricLinesTemplateCache = {
    scoreData: this._scoreData, template: lyricLinesTemplate, entries: entries,
  };
  return entries;
}

// A supplied template resolved to this run's own syllables, as break indices, or null to
// leave the run to whatever would have decided it. Guarded like _printedBreaksForRun: a
// break must land on a syllable that opens a word, and a stanza whose positions can't all be
// placed falls back rather than half-applying. The list is song-wide, so only positions
// inside this run's span and addressed to a lyric line it sings are its business.
ChScore.prototype._lyricLineBreaksForRun = function (run, sungChordPositions = null) {
  const entries = this._parseLyricLinesTemplate(this._suppliedTemplates.lyricLines);
  if (!entries || entries.length === 0) return null;

  // Every lyric line this run sings, not just the one it is named for: a section can take two
  // of them, one after the other, and a break addressed to either is this run's business.
  const lyricLineIds = new Set([run.lyricLineId ?? '']);
  for (const syllable of run.syllables) {
    for (const id of syllable.lyricLineIds ?? []) lyricLineIds.add(id);
  }
  const wanted = new Set(entries
    .filter(entry => !entry.lyricLineIds || entry.lyricLineIds.some(id => lyricLineIds.has(id)))
    .map(entry => entry.chordPosition));
  if (wanted.size === 0) return null;

  // Walked by syllable rather than looked up by chord position, because a run sings through
  // a repeat: a chorus written once and played twice comes past the same positions again,
  // and a break belongs to every pass, not just the first.
  const breaks = [];
  ...
  for (const [index, syllable] of run.syllables.entries()) {
    const chordPosition = syllable.chordPositions[0];
    lowest = Math.min(lowest, chordPosition);
    highest = Math.max(highest, chordPosition);
    if (index === 0 || !wanted.has(chordPosition)) continue;
    // Nothing may break a word across two lines, and a template that asks for it is
    // addressed to some other engraving of the song
    if (!this._syllableStartsWord(syllable)) return null;
    reached.add(chordPosition);
    breaks.push(index);
  }

  // A position outside the run's span belongs to another stanza; one inside it that this run
  // doesn't sing belongs to whatever does, since a span is not solid -- a chorus given its own
  // section brackets positions the verses sing and it does not. What says the template was
  // written against another engraving is a position *nothing* sings, so that is what is asked,
  // and the stanza falls back rather than half-applying its breaks.
  for (const chordPosition of wanted) {
    if (chordPosition <= lowest || chordPosition > highest) continue;
    if (reached.has(chordPosition)) continue;
    if (sungChordPositions?.has(chordPosition)) continue;
    return null;
  }
...
ChScore.prototype._getPhraseStartsByStaff = function (syllables, runs = null,
  ...
  const groups = this._scoreData.features?.hasTwoPartMelody
    ...
    : [runs];
  for (const group of groups) {
    const segmented = this._segmentRuns(syllables, group, applyTemplate);
    ...
    for (const [chordPosition, open] of segmented.openInAllLines) {
      openInAllLines.set(chordPosition, open);
    }
  }
...
ChScore.prototype._alignPhraseStartsToRun = function (phraseStarts, run) {
  ...
  const opensWord = (syllable) => this._syllableStartsWord(syllable);
  ...
  for (const [index, syllable] of run.syllables.entries()) {
    ...
    if (!opensWord(syllable)) {
      ...
      // line short -- so it moves to the nearest syllable that opens one, earlier for
      // preference. Usually that is a syllable away, which is all the room two verses sung
      // from one staff ever need; a word set to these notes can be longer than two syllables,
      // so the search widens until it finds one, bounded by how long a line wants to be.
      let moved = null;
      for (let away = 1; away <= CH_COMFORTABLE_LINE_SYLLABLES && moved == null; away++) {
        moved = [index - away, index + away]
          .find(at => at > 0 && at < run.syllables.length && opensWord(run.syllables[at])) ?? null;
      }
      if (moved != null) {
        chosen.push({ index: moved, chordPosition: run.syllables[moved].chordPositions[0],
          templated: Boolean(templated?.has(syllable.chordPositions[0])) });
      }
      continue;
    }
    ...
    chosen.push({ index: best, chordPosition: run.syllables[best].chordPositions[0],
      templated: Boolean(templated?.has(syllable.chordPositions[0])) });
  }

  // One break to a neighbourhood, where a neighbourhood is as long as a line wants to be:
  // two shared positions a syllable apart are two verses disagreeing about one break, not a
  // line of one syllable. Which of them stands: a position every stanza sung there could
  // break at first, then one this verse asked for, then the one its own words score higher.
  const openInAll = (candidate) =>
    phraseStarts.openInAllLines?.get(candidate.chordPosition) ?? true;
  const prefers = (candidate, against) => {
    if (openInAll(candidate) !== openInAll(against)) return openInAll(candidate);
    const candidateAsked = !asked || asked.has(candidate.chordPosition);
    const againstAsked = !asked || asked.has(against.chordPosition);
    if (candidateAsked !== againstAsked) return candidateAsked;
    return ownScore(candidate.index) > ownScore(against.index);
  };
  ...
  for (const candidate of chosen) {
    ...
    if (previous && candidate.index - previous.index < CH_COMFORTABLE_LINE_SYLLABLES) {
      // None of this weighs on a break the caller's template asked for: two of those close
      // together is a short line the caller wants ("Sh, be still."), not two readings of one
      // break. Only where neither is the caller's is the shorter reading taken.
      if (candidate.templated && previous.templated) {
        kept.push(candidate);
        continue;
      }
      if (candidate.templated !== previous.templated) {
        if (candidate.templated) kept[kept.length - 1] = candidate;
        continue;
      }
      if (prefers(candidate, previous)) kept[kept.length - 1] = candidate;
      continue;
    }
    kept.push(candidate);
  }
  for (const { chordPosition } of kept) aligned.add(chordPosition);
  return aligned;
}
...
// How long a line wants to be. Not a floor: a line shorter than this pays shortLineCost for
// every syllable it lacks, wherever in the stanza it falls, so a closing tag ("Evermore!") is
// affordable where the evidence is there and a stray two-syllable line still isn't. It is also
// how close two breaks may land in _alignPhraseStartsToRun before the shorter reading is taken
// as a disagreement rather than a line -- the same question, asked where nothing pays for it.
const CH_COMFORTABLE_LINE_SYLLABLES = 4;

// One group of stanzas segmented against its own evidence and its own typical line length.
ChScore.prototype._segmentRuns = function (syllables, runs, applyTemplate = false) {
  ...
  const comfortableLength = CH_COMFORTABLE_LINE_SYLLABLES;
  const maxLength = 16;
  ...
  const segment = (run, targetLength, lengthWeight = lengthCost) => {
    ...
    const opensWord = (index) => this._syllableStartsWord(run.syllables[index]);
  ...
  // Whether every stanza sung at a position can open a line there. A break has to land on a
  // syllable that opens a word, and stacked verses fit different words to the same notes: in
  // "Behold the Wounds in Jesus' Hands" the note after 'madera,' carries a whole word in
  // verses 1 and 3 ('Él', 'hoy') and the tail of one in verses 2 and 4 ('dad;', 'rá'). A
  // position half the verses cannot break at is a poor answer for the music they share, and
  // that is what tells it from a position they merely disagree about (see
  // _alignPhraseStartsToRun).
  const openInAllLines = new Map();
  for (const run of runs) {
    for (const syllable of run.syllables) {
      const chordPosition = syllable.chordPositions[0];
      const opens = this._syllableStartsWord(syllable);
      openInAllLines.set(chordPosition, (openInAllLines.get(chordPosition) ?? true) && opens);
    }
  }

  // Every position anything sings at, which is what tells a template written against another
  // engraving from one whose positions simply belong to another stanza
  const sungChordPositions = new Set();
  for (const syllable of syllables) {
    for (const chordPosition of syllable.chordPositions ?? []) sungChordPositions.add(chordPosition);
  }
  ...
  const shared = new Set();
  const askedByLine = new Map();
  // The subset of those that came from the caller's template, kept apart because they are
...
ChScore.prototype._patterns = {
  ...
  // The same class the other way round: anything that is not punctuation
  letterOrNumber: new RegExp(ChScore.prototype._punctuationCharacter.replace('^', ''), 'u'),

  // A syllable that is nothing but a dash: a verse holding still where another sings ("—"
  // under the pickup verse 1 sings), not a word
  holdDash: /^[—–]+$/,
...
ChScore.prototype._insertKnownHyphens = function (word, offsets = null) {
  let result = word;
  for (const at of (offsets ?? this._knownHyphenOffsets(word)).slice().reverse()) {
...
// guitar chords over a lyrics view, say -- has somewhere to go wherever the section plays. A
// wordless section, the run before its first syllable, and music _fillSectionGaps added all
// lack one; unbroken runs of them are marked a run at a time. lyricsText is untouched.
ChScore.prototype._markSectionChordPositions = function () {
  for (const section of this._scoreData.sections ?? []) {
    // The positions its markers already anchor, read back off them
    const marked = new Set();
    for (const match of String(section.lyricsAnnotated ?? '').matchAll(/data-ch-chord-position="([^"]*)"/g)) {
      for (const chordPosition of match[1].split(' ')) {
        if (chordPosition) marked.add(Number.parseInt(chordPosition));
      }
    }
    const unmarked = [];
    for (const chordPositionRange of section.chordPositionRanges ?? []) {
      for (let cp = chordPositionRange.start; cp < chordPositionRange.end; cp++) {
        if (!marked.has(cp)) unmarked.push(cp);
      }
    }
    if (unmarked.length === 0) continue;

    // One marker per unbroken run, naming every position in it, and every expanded position
    // this section plays them at -- the same reading a syllable held over several positions
    // gets. An unexpanded score answers with the printed positions themselves.
    const markersFor = (chordPositions) => {
      let markers = '';
      for (let at = 0; at < chordPositions.length;) {
        let to = at + 1;
        while (to < chordPositions.length && chordPositions[to] === chordPositions[to - 1] + 1) to += 1;
        const run = chordPositions.slice(at, to);
        const expandedChordPositions = [];
        for (const cp of run) {
          const played = this._scoreData.chordPositions[cp]?.expandedChordPositions?.[section.sectionId] ?? [];
          for (const ecp of played) expandedChordPositions.push(ecp);
        }
        markers += this._syllableMarker({ chordPositions: run, expandedChordPositions });
        at = to;
      }
      return markers;
    };
    if (!section.lyricsAnnotated) {
      section.lyricsAnnotated = markersFor(unmarked);
      continue;
    }
    // Played before the first word, or after the last: the words themselves stay put
    const firstMarked = Math.min(...marked);
...
// A syllable's marker: an empty span saying where it is sung, which is what makes the
// annotated reading of the lyrics followable during playback.
ChScore.prototype._syllableMarker = function (syllable) {
  return `<span data-ch-chord-position="${(syllable.chordPositions ?? []).join(' ')}"`
    + ` data-ch-expanded-chord-position="${(syllable.expandedChordPositions ?? []).join(' ')}"`
...
ChScore.prototype._annotateWord = function (segments, word, hyphenOffsets) {
  const markersByOffset = new Map();
  let offset = 0;
  for (const segment of segments) {
    if (segment.marker) markersByOffset.set(offset, (markersByOffset.get(offset) ?? '') + segment.marker);
    offset += segment.text.length;
  }

  let annotated = '';
  for (let at = 0; at <= word.length; at++) {
    if (hyphenOffsets.has(at)) annotated += '-';
    if (markersByOffset.has(at)) annotated += markersByOffset.get(at);
    if (at < word.length) annotated += word[at];
  }
  return annotated;
}
...
ChScore.prototype._wordBuilder = function () {
  ...
  return {
    ...
    text() { return this.render(false); },
    annotatedText() { return this.render(true); },
  };
}

// Whether a run is an anacrusis leading into the one after it, read off the music
...
ChScore.prototype._isPickupFragment = function (run, next) {
  ...
  if (run.syllables.at(-1).expandedChordPositions.at(-1) + 1 !== next.syllables[0].expandedChordPositions[0]) {
    return false;
  }
  if (nextStart >= span.end) return false;
  ...
  const measureIndex = this._scoreData.chordPositions[span.start]?.measureIndex;
  return measureIndex != null
    && measureIndex === this._scoreData.chordPositions[span.end - 1]?.measureIndex;
}

// Merge pickup fragments into the verse they belong to, before the words are built. A hymn
// often engraves the next verse's first syllables on a pickup before a repeat, so they arrive
// as a run of their own — sung before the verse they belong to, and split off from it by the
// jump back into the repeat. Which verse that is can be printed ("2." on lyric line 1, "Were
...
// question; the other two are settled by _isPickupFragment reading the music. `joins` collects
// each verse's first syllable after a pickup, where no line may break.
ChScore.prototype._mergePickupRuns = function (runs, joins) {
  const markerOf = (run) => run?.syllables.find(syllable => syllable.label)?.label ?? null;
  const merged = [];
  let pickup = [];

  for (let r = 0; r < runs.length; r++) {
    const run = pickup.length ? { ...runs[r], syllables: [...pickup, ...runs[r].syllables] } : runs[r];
    pickup = [];
    const next = runs[r + 1];
    const number = this._markerNumber(markerOf(run));
    const lineNumber = this._verseLineNumber(run.lyricLineId);
    const nextLineNumber = this._verseLineNumber(next?.lyricLineId);

    // Whatever the label says, the fragment can only join a stanza of its own kind that
    // hasn't been numbered as a verse in its own right. Where sections were given and the next
    // one starts after a fragment that starts none, the fragment's type is only the section
    // it is engraved in, and says nothing either way.
    const opensNextSection = next?.syllables[0]?.startsSection && !run.syllables[0]?.startsSection;
    const nextTakesPickup = Boolean(next)
      && (next.type === run.type || opensNextSection)
      && this._markerNumber(markerOf(next)) === null;
    ...
    const labelled = number !== null && nextLineNumber === number
      && (number !== lineNumber || this._isPickupFragment(run, next));

    // No label at all: the music is the only evidence there is.
    const unlabelled = number === null && nextLineNumber === lineNumber
      && this._isPickupFragment(run, next);

    if (nextTakesPickup && (labelled || unlabelled)) {
      joins.add(next.syllables[0]);
      pickup = run.syllables;
      continue;
    }
    merged.push(run);
  }
...
// What a stanza is called: "Verse 2", "Chorus", or "Section" where the score says
...
ChScore.prototype._stanzaName = function (stanza) {
  ...
  if (stanza.type === 'verse') {
    ...
    return `Verse ${stanza.marker || this._verseLineNumber(stanza.lyricLineIds?.[0]) || ''}`.trim();
  }
  // Types are stored lowercase ("chorus", "bridge") and displayed capitalized
  return stanza.type ? stanza.type.charAt(0).toUpperCase() + stanza.type.slice(1) : 'Section';
}
...
// A template position as this score reads it, or null where it names a measure or chord
// position the score hasn't got. A chord position is exact against the score it came from. A
// `measure@beat` one is written against another engraving, which can set a syllable earlier or
// later, so one partway through a word sung on the lines named (or any, where none are) moves a
// syllable earlier or, failing that, later, to where every word sung there starts -- but never
// from or across a repeated section or ending's start, where a translation can split a word over
// the jump ("a-" before a first ending, "quí" in each). `syllableIndex` returns
// _templateSyllableIndex's answer, asked for only when snapping; without one nothing snaps (a
// lyric line break has its own rule, in _lyricLineBreaksForRun).
ChScore.prototype._resolveTemplatePosition = function (text, numChordPositions,
  ...
  const sungAt = (at) => {
    const sung = byChordPosition.get(at) ?? [];
    const named = sung.filter(syllable => lyricLineIds?.includes(syllable.lineId));
    return named.length > 0 ? named : sung;
  };
  const opensAll = (at) => sungAt(at).every(syllable => syllable.opensWord);
  if (opensAll(chordPosition) || jumps.has(chordPosition)) return chordPosition;
  const positions = byLine.get(sungAt(chordPosition).find(syllable => !syllable.opensWord).lineId);
  const index = positions.indexOf(chordPosition);
  return [positions[index - 1], positions[index + 1]].find(at => at !== undefined && opensAll(at)
    && ![...jumps].some(jump => jump > Math.min(at, chordPosition) && jump <= Math.max(at, chordPosition)))
    ?? chordPosition;
}

// Every syllable's chord position, line and whether it opens a word, and where each repeated
// section or ending starts, for _resolveTemplatePosition. Once the lyrics are annotated, only
// the tune's syllables: a part echoing the words can be partway through one where the tune
// starts a new one, and kept per document, since several template readers ask. Before (the
// parts template is read first), every staff's, placed through `byElementId`, with lines named
// by their row.
ChScore.prototype._templateSyllableIndex = function (byElementId = null) {
  const meiParsed = this._scoreData.meiParsed;
  const cache = this._templateSyllableIndexCache;
  if (!byElementId && cache?.meiParsed === meiParsed) return cache.index;

  const chordPositionOf = (element) => {
    if (element.hasAttribute('ch-chord-position')) return Number.parseInt(element.getAttribute('ch-chord-position'));
    const note = element.localName === 'chord' ? element.querySelector('note') : element;
    return byElementId?.[note?.getAttribute('xml:id')];
  };
  const jumps = new Set();
  for (const part of meiParsed.querySelectorAll('section section, ending')) {
    const first = part.querySelector('note, rest, chord');
    const chordPosition = first && chordPositionOf(first);
    if (Number.isInteger(chordPosition)) jumps.add(chordPosition);
  }
  const sung = byElementId
    ? Array.from(meiParsed.querySelectorAll('verse:not([ch-help-text]):has(syl:not(:empty))'),
      lyricElement => {
        const holder = lyricElement.closest('note, chord');
        return [holder && chordPositionOf(holder), lyricElement];
      })
    : [...this._melodyLyricElementIndex().byChordPosition]
      .flatMap(([chordPosition, lyricElements]) => lyricElements.map(lyricElement => [chordPosition, lyricElement]));
  const byChordPosition = new Map();
  const byLine = new Map();
  for (const [chordPosition, lyricElement] of sung) {
    if (!Number.isInteger(chordPosition)) continue;
    const lineId = lyricElement.getAttribute('ch-lyric-line-id')
      ?? `${lyricElement.closest('staff')?.getAttribute('n')}.${lyricElement.getAttribute('n')}`;
    if (!byChordPosition.has(chordPosition)) byChordPosition.set(chordPosition, []);
    const text = Array.from(lyricElement.querySelectorAll('syl'), syl => syl.textContent).join('').trim();
    byChordPosition.get(chordPosition).push({ lineId, opensWord: this._startsWord(lyricElement),
      isHold: this._patterns.holdDash.test(text) });
    if (!byLine.has(lineId)) byLine.set(lineId, []);
    byLine.get(lineId).push(chordPosition);
  }
  for (const positions of byLine.values()) positions.sort((a, b) => a - b);
  const index = { byChordPosition, byLine, jumps };
  if (!byElementId) this._templateSyllableIndexCache = { meiParsed, index };
  return index;
}

// Whether a stanza's range carries on over the gap from `from` up to `to` rather than opening
// another: where other verses sing a few lone syllables and the stanza's own lines only hold (a
// "—"), it is holding still, not skipping music. Not where nothing is sung, or a repeated
// section or ending starts, which is music of its own.
ChScore.prototype._continuesOverGap = function (from, to, lyricLineIds) {
  if (!this._scoreData || !lyricLineIds?.length || to - from > this._maxLoneSyllables) return false;
  const { byChordPosition, jumps } = this._templateSyllableIndex();
  if (jumps.has(to)) return false;
  for (let chordPosition = from; chordPosition < to; chordPosition++) {
    const syllables = byChordPosition.get(chordPosition) ?? [];
    if (jumps.has(chordPosition) || syllables.length === 0
      || syllables.some(syllable => !syllable.isHold && lyricLineIds.includes(syllable.lineId))) return false;
  }
  return true;
}
...
ChScore.prototype._chordPositionQstamps = function () {
  const chordPositions = this._scoreData.chordPositions ?? [];
  if (this._chordPositionQstampCache?.chordPositions !== chordPositions
    || this._chordPositionQstampCache.qstamps.length !== chordPositions.length) {
    this._chordPositionQstampCache = {
      chordPositions: chordPositions,
      qstamps: chordPositions.map(chordPositionInfo => chordPositionInfo.startQ),
    };
  }
  return this._chordPositionQstampCache.qstamps;
}
